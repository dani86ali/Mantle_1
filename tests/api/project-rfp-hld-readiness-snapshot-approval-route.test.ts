...
// Mock auth and the HLD readiness snapshot approval service so the route's
// auth gate, body validation, tenant/user/param authority, body-ignoring, and
// result mapping are tested independent of the DB and the project/artifact/
// approval stores.
...
vi.mock("@/lib/projects/project-rfp-hld-readiness-snapshot-approval", () => ({
  reviewRfpHldReadinessSnapshotArtifact: mockReview,
}));

import { POST } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-readiness-snapshot/review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-readiness-snapshot/review/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-hld-readiness-snapshot-3";
const SESSION = {
  userId: "u-engineer",
...
describe("POST .../rfp/artifacts/[artifactId]/hld-readiness-snapshot/review - auth", () => {
...
describe("POST .../hld-readiness-snapshot/review - body validation", () => {
  it("returns 400 invalid_rfp_hld_readiness_snapshot_review_request for invalid JSON without calling the service", async () => {
    const res = await POST(req(undefined, { invalidJson: true }), PARAMS);

    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.code).toBe("invalid_rfp_hld_readiness_snapshot_review_request");
  ...
  it("returns 400 for invalid bodies without calling the service", async () => {
    ...
    for (const body of badBodies) {
      ...
      expect(json.code).toBe("invalid_rfp_hld_readiness_snapshot_review_request");
      expect(mockReview).not.toHaveBeenCalled();
    }
  });
});

describe("POST .../hld-readiness-snapshot/review - authority", () => {
  it("passes only session tenant/user, route params, and decision/note; decoy authority fields never reach the service", async () => {
    ...
    expect(arg.note).toBe("not ready");
    ...
    for (const leaked of ["decidedAt", "status", "payload"]) {
      expect(leaked in arg).toBe(false);
    }
  });

  it("preserves an optional string note", async () => {
    await POST(req(validBody({ note: "looks ready for HLD" })), PARAMS);

    const arg = mockReview.mock.calls[0][0] as Record<string, unknown>;
    expect(arg.note).toBe("looks ready for HLD");
  });
...
describe("POST .../hld-readiness-snapshot/review - result mapping", () => {
  const SIMPLE: Array<[Record<string, unknown>, number, string]> = [
    [{ status: "not_found" }, 404, "project_not_found"],
    [{ status: "artifact_not_found" }, 404, "hld_readiness_snapshot_artifact_not_found"],
    [{ status: "approval_failed" }, 409, "hld_readiness_snapshot_review_failed"],
  ];
  ...
  it("maps artifact_not_hld_readiness_snapshot to 409 with the artifact summary", async () => {
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("artifact_not_hld_readiness_snapshot");
    expect(body.artifact).toEqual(artifact);
  });

  it("maps artifact_not_reviewable to 409 hld_readiness_snapshot_artifact_not_reviewable with the artifact summary", async () => {
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_readiness_snapshot_artifact_not_reviewable");
    expect(body.error).toBe("HLD readiness snapshot artifact is not reviewable.");
    expect(body.artifact).toEqual(artifact);
  });

  it("maps invalid_hld_readiness_snapshot_payload to 409 hld_readiness_snapshot_payload_invalid with the artifact summary", async () => {
    mockReview.mockResolvedValue({
      status: "invalid_hld_readiness_snapshot_payload",
      artifact: ARTIFACT_SUMMARY,
    });
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_readiness_snapshot_payload_invalid");
...
describe("POST .../hld-readiness-snapshot/review - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    const secret = "boom-internal-stack-review";
    ...
    const body = await res.json();
    expect(body.code).toBe("hld_readiness_snapshot_review_failed");
    expect(body.error).toBe("Unable to review HLD readiness snapshot.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../hld-readiness-snapshot/review - route surface", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-readiness-snapshot/review/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-hld-readiness-snapshot-approval-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the HLD readiness snapshot approval service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-hld-readiness-snapshot-approval",
    ]);
  });
