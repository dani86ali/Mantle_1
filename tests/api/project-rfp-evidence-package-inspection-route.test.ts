...
// Mock auth and the read-only evidence_package inspection service so the
// routes' auth gates, tenant/param authority, body-ignoring, and result
// mapping are tested independent of the DB and the evidence-package layers.
...
vi.mock("@/lib/projects/project-rfp-evidence-package-inspection", () => ({
  loadRfpEvidencePackageList: mockLoadList,
  loadRfpEvidencePackageDetail: mockLoadDetail,
}));

import { GET as listGET } from "@/app/api/projects/[id]/rfp/evidence-package/route";
import * as listRouteModule from "@/app/api/projects/[id]/rfp/evidence-package/route";
import { GET as detailGET } from "@/app/api/projects/[id]/rfp/evidence-package/[artifactId]/route";
import * as detailRouteModule from "@/app/api/projects/[id]/rfp/evidence-package/[artifactId]/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-evidence-package-1";
...
// Distinctive sentinels: the final evidence content under human review. The
// detail must expose both verbatim; the lean list summary must expose neither.
const EVIDENCE_TEXT = "Provide 48-port access switches.";
const TABLE_CELL = "Access switch requirement cell";
...
// The lean list payload summary: identifiers and counts only. It never carries
// the evidence array, a text body, a table row, a tenantId, or a storage path.
...
// The detail read model: the sanitized final evidence content. Because this is
// what a human reviews and approves, it carries the chunk text bodies and the
// fresh table row matrices verbatim, while still dropping any tenantId/storage
// path. The detail mapping is proven to pass this through unchanged.
  ...
  evidence: [
    {
      ...
      documentMetrics: {
        textCharCount: 320,
        nonWhitespaceTextCharCount: 280,
        tableCount: 1,
        tableRowCount: 4,
      },
    },
    {
      ...
      inputPackageArtifactId: INPUT_PACKAGE_ID,
      sourceFileName: "file-sow-1.xlsx",
      sourceFileRole: "sow",
      tableId: "tbl-1",
      pageNumber: 2,
...
describe("GET .../rfp/evidence-package - auth", () => {
...
describe("GET .../rfp/evidence-package - authority", () => {
...
describe("GET .../rfp/evidence-package - result mapping", () => {
  ...
  it("maps ok to 200 with { project, artifactCount, artifacts }, no status, and no evidence body/tenant/storage", async () => {
    const res = await listGET(req(), LIST_PARAMS);
    ...
    // The lean list item carries only the payload summary - no evidence array,
    // no text body, no table rows.
    const item = body.artifacts[0];
    expect("evidence" in item).toBe(false);
    expect("evidence" in item.payloadSummary).toBe(false);
    expect("text" in item.payloadSummary).toBe(false);
    expect("rows" in item.payloadSummary).toBe(false);
    const json = JSON.stringify(body);
    expect(json).not.toContain('"evidence":');
    expect(json).not.toContain('"text":');
    expect(json).not.toContain('"rows":');
    expect(json).not.toContain(EVIDENCE_TEXT);
    expect(json).not.toContain(TABLE_CELL);
  ...
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    expect(body).toEqual({
      ...
      error: "Unable to inspect evidence packages.",
    });
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("GET .../rfp/evidence-package/[artifactId] - auth", () => {
  it("returns the requireAuth response and never calls the service or reads the body when unauthenticated", async () => {
    const unauth = NextResponse.json(
      { error: "Authentication required" },
      { status: 401 }
...
describe("GET .../rfp/evidence-package/[artifactId] - authority", () => {
...
describe("GET .../rfp/evidence-package/[artifactId] - result mapping", () => {
  it("maps not_found to 404 project_not_found", async () => {
  ...
  it("maps artifact_not_found to 404 evidence_package_artifact_not_found", async () => {
  ...
  it("maps artifact_not_evidence_package to 409 with the artifact summary", async () => {
    ...
    expect(body.error).toBe("Artifact is not an evidence_package artifact.");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
  });

  it("maps invalid_payload to 409 evidence_package_invalid_payload with the artifact summary", async () => {
    mockLoadDetail.mockResolvedValue({
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("evidence_package_invalid_payload");
    expect(body.error).toBe("Evidence package payload is invalid.");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
  });

  it("maps ok to 200 with { project, artifact, package }; text bodies and table rows kept, tenant/storage never", async () => {
    const res = await detailGET(req(), DETAIL_PARAMS);
    ...
    expect("status" in body).toBe(false);
    // The read model is the final evidence content under human review, so the
    // detail keeps the chunk text bodies and table row matrices verbatim; the
    // route adds or strips nothing.
    expect(body.package.evidence[0].text).toBe(EVIDENCE_TEXT);
    expect(body.package.evidence[1].rows).toEqual([["Item", TABLE_CELL]]);
    const json = JSON.stringify(body);
    expect(json).toContain(EVIDENCE_TEXT);
    expect(json).toContain(TABLE_CELL);
    expect(json).not.toContain("tenantId");
    expect(json).not.toContain(SESSION.tenantId);
    expect(json).not.toContain("storagePath");
  });
...
describe("route module purity (static source check)", () => {
  const LIST_SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/evidence-package/route.ts"
  );
  const DETAIL_SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/evidence-package/[artifactId]/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-evidence-package-inspection-route.test.ts"
  );
  ...
  it.each(sources)(
    "the %s route imports no DB/store, no evidence-package write/approval/run/persistence service, no extraction-delta/candidate-drafting/executor service, and no requirements/boq/pricing/SKU/config/export/catalog/runner/AI/agent/coordinator/engine/UI/app module",
    (_label, source) => {
      // require( and fetch( use the open paren so requireAuth and request are
      // not false positives; the evidence-package write/approval/run/
      // persistence and the extraction-delta modules use a closing quote (and
      // the candidate-drafting family a path prefix), so the legitimately
      // imported evidence-package -inspection service passes.
