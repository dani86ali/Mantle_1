...
vi.mock("@/lib/projects/project-quick-bom-upload", () => ({
  uploadQuickBomBoqFile: mockUpload,
}));

import { POST } from "@/app/api/projects/[id]/quick-bom/files/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/files/route";
...
const OK_FILE = {
  ...
  storagePath: "/tmp/bomatic-project-uploads/t/p/u/Honeywell_BoQ.xlsx",
  mimeType: "text/csv",
  sizeBytes: 3,
...
function mockFile(name: string, content: string, type = "text/csv"): File {
...
describe("POST /api/projects/[id]/quick-bom/files - auth", () => {
  it("returns the requireAuth response and skips the service when unauthenticated", async () => {
    ...
    const res = await POST(multipartReq(fileForm(mockFile("boq.csv", "abc"))), {
      params: { id: PROJECT },
    });
...
describe("POST /api/projects/[id]/quick-bom/files - request validation", () => {
  it("returns 400 and skips the service when request.formData() rejects", async () => {
    ...
    expect(body.code).toBe("invalid_quick_bom_boq_upload_request");
    expect(mockUpload).not.toHaveBeenCalled();
  });

  it("returns 400 and skips the service for a missing file field", async () => {
    const res = await POST(multipartReq(new FormData()), {
      params: { id: PROJECT },
    });

    expect(res.status).toBe(400);
    expect((await res.json()).code).toBe("invalid_quick_bom_boq_upload_request");
    expect(mockUpload).not.toHaveBeenCalled();
  });
  ...
  it("returns 400 and skips the service for duplicate file fields", async () => {
    const form = new FormData();
    form.append("file", mockFile("a.csv", "a"));
    form.append("file", mockFile("b.csv", "b"));
...
describe("POST /api/projects/[id]/quick-bom/files - tenant/param authority", () => {
  it("passes session.tenantId, route param id, real file metadata and bytes, and ignores decoy body fields", async () => {
    const content = "WXYZ";
    const expectedBytes = Array.from(new TextEncoder().encode(content));
    const form = new FormData();
    form.append("file", mockFile("Honeywell BoQ.xlsx", content, "application/xlsx"));
    // Decoy fields that must never reach the service.
    form.append("tenantId", "attacker-tenant");
    form.append("projectId", "attacker-project");
    form.append("mode", "rfp");
    ...
    expect(arg.fileName).toBe("Honeywell BoQ.xlsx");
    expect(arg.mimeType).toBe("application/xlsx");
...
describe("POST /api/projects/[id]/quick-bom/files - result mapping", () => {
  ...
  it("maps service invalid_file to 400 invalid_quick_bom_boq_file", async () => {
    mockUpload.mockResolvedValue({
      status: "invalid_file",
    ...
    const res = await POST(
      multipartReq(fileForm(mockFile("legacy.xls", "abc"))),
      { params: { id: PROJECT } }
    );

    expect(res.status).toBe(400);
    expect((await res.json()).code).toBe("invalid_quick_bom_boq_file");
  });
...
describe("POST /api/projects/[id]/quick-bom/files - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_boq_upload_failed");
    expect(body.error).toBe("Unable to upload Quick BoM BoQ file.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST /api/projects/[id]/quick-bom/files - route surface", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-quick-bom-upload-route.test.ts"
  );
  ...
  it("imports only Next.js server primitives, requireAuth, and the upload service", () => {
    ...
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-upload",
    ]);
  });

  it("does not import DB, mutation stores, BoQ loaders/parsers, normalization, artifact/approval/evidence stores, pricing, config expansion, export, runner, AI, catalog, engine, coordinator, or adapter modules", () => {
