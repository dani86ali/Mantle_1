...
// Mock auth and the Quick BoM export wrapper service so the route's auth-gate,
// tenant/param authority, body-ignoring, and result-mapping are tested independent of
// the DB and the lower-level Mantle export service.
...
vi.mock("@/lib/projects/project-quick-bom-export", () => ({
  createProjectQuickBomExportPackage: mockCreateExport,
}));

import { POST } from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/export-package/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/export-package/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-1";
const ARTIFACT_ID = "art-pb-7";
const SESSION = {
  userId: "u-engineer",
...
// authority/path/category fields so a test can prove neither is read.
...
describe("POST .../export-package - auth", () => {
...
describe("POST .../export-package - tenant/param authority", () => {
  it("uses session.tenantId plus route params only and never reads the request body", async () => {
    ...
    // No body-supplied path/category authority ever reaches the service.
    expect("outputPath" in arg).toBe(false);
    expect("filePath" in arg).toBe(false);
    expect("categoryByAcceptedSku" in arg).toBe(false);
...
describe("POST .../export-package - result mapping", () => {
...
describe("POST .../export-package - service failure", () => {
  it("maps an unexpected service error to a controlled 500 distinct from the workbook 500, without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_export_package_failed");
    expect(body.code).not.toBe("export_workbook_failed");
    expect(body.error).toBe("Unable to create Quick BoM export package.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../export-package - route surface", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(process.cwd(), "tests/api/project-quick-bom-export-route.test.ts");
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the export wrapper service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-export",
    ]);
  });

  it("does not import DB, stores, the export service/helpers, mantle/fixture, runner, AI, catalog, engine, coordinator, or adapter modules", () => {
  ...
  it("does not read the request body (no request.json/formData/text)", () => {
