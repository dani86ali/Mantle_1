...
vi.mock("@/lib/projects/project-rfp-hld-intake-inspection", () => ({
  loadRfpHldIntakeDetail: mockLoadDetail,
}));

import { GET } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-intake/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-intake/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-hld-intake-2";
const SESSION = {
  userId: "u-engineer",
...
// The sanitized whitelist copy of the stored intake payload: engineer-authored
// answer text/notes are allowed (intake is design context, not raw RFP text).
const INTAKE = {
  ...
  answers: [
    {
      ...
      label: "Existing network context",
      status: "answered",
      value: "Two legacy core switches in DC1.",
    },
    {
      fieldId: "diagram_notes",
      label: "Diagram notes",
      status: "unknown",
    },
  ],
...
describe("GET .../rfp/artifacts/[artifactId]/hld-intake - auth", () => {
...
describe("GET .../hld-intake (detail) - authority", () => {
...
describe("GET .../hld-intake (detail) - result mapping", () => {
  ...
  it("maps artifact_not_found to 404 hld_intake_artifact_not_found", async () => {
  ...
  it("maps artifact_not_hld_intake to 409 with the lean artifact summary", async () => {
    const artifact = {
      ...ARTIFACT_SUMMARY,
      type: "input_package",
  ...
  it("maps invalid_payload to 409 hld_intake_invalid_payload with the lean artifact summary", async () => {
    mockLoadDetail.mockResolvedValue({
  ...
  it("maps ok to 200 with { project, artifact, intake }, no status discriminator, and no tenantId leakage", async () => {
    const res = await GET(req(), PARAMS);
    ...
    // Engineer-authored answer text is allowed in the detail...
    expect(json).toContain("Two legacy core switches in DC1.");
    // ...but tenant and storage leakage never is.
...
describe("GET .../hld-intake (detail) - route surface", () => {
  it("exports GET only", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-intake/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-hld-intake-detail-route.test.ts"
  );
