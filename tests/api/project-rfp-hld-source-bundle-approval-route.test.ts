...
// Mock auth and the HLD source-bundle approval service so the route's auth gate,
...
vi.mock("@/lib/projects/project-rfp-hld-source-bundle-approval", () => ({
  reviewRfpHldSourceBundleArtifact: mockReview,
}));

import { POST } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-source-bundle/review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-source-bundle/review/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-hld-source-bundle-9";
const SESSION = {
  userId: "u-engineer",
...
describe("POST .../rfp/artifacts/[artifactId]/hld-source-bundle/review - auth", () => {
...
describe("POST .../hld-source-bundle/review - body validation", () => {
  it("returns 400 invalid_rfp_hld_source_bundle_review_request for invalid JSON without calling the service", async () => {
    const res = await POST(req(undefined, { invalidJson: true }), PARAMS);

    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.code).toBe("invalid_rfp_hld_source_bundle_review_request");
  ...
  it("returns 400 for invalid bodies without calling the service", async () => {
    ...
    for (const body of badBodies) {
      ...
      expect(json.code).toBe("invalid_rfp_hld_source_bundle_review_request");
      expect(mockReview).not.toHaveBeenCalled();
    }
  });
});

describe("POST .../hld-source-bundle/review - authority", () => {
...
describe("POST .../hld-source-bundle/review - result mapping", () => {
  const SIMPLE: Array<[Record<string, unknown>, number, string]> = [
    [{ status: "not_found" }, 404, "project_not_found"],
    [{ status: "artifact_not_found" }, 404, "hld_source_bundle_artifact_not_found"],
    [{ status: "approval_failed" }, 409, "hld_source_bundle_review_failed"],
  ];
  ...
  it("maps artifact_not_hld_source_bundle to 409 with the artifact summary", async () => {
    const artifact = { ...ARTIFACT_SUMMARY, type: "hld_intake" };
    mockReview.mockResolvedValue({
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("artifact_not_hld_source_bundle");
    expect(body.artifact).toEqual(artifact);
  });

  it("maps artifact_not_reviewable to 409 hld_source_bundle_artifact_not_reviewable with the artifact summary", async () => {
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_source_bundle_artifact_not_reviewable");
    expect(body.error).toBe("HLD source bundle artifact is not reviewable.");
    expect(body.artifact).toEqual(artifact);
  });

  it("maps invalid_hld_source_bundle_payload to 409 hld_source_bundle_payload_invalid with the artifact summary", async () => {
    mockReview.mockResolvedValue({
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_source_bundle_payload_invalid");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
  });

  it("maps stale_hld_source_bundle_payload to 409 hld_source_bundle_payload_stale with staleCode, messages, and errors", async () => {
    mockReview.mockResolvedValue({
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_source_bundle_payload_stale");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
    expect(body.staleCode).toBe("recompute_blocked");
    expect(body.messages).toEqual([
      "An approved HLD readiness snapshot is required before the source bundle.",
    ]);
    expect(body.errors).toEqual(["e1", "e2"]);
  });

  it("maps stale_hld_source_bundle_payload with no messages/errors to a clean 409 (omits both keys)", async () => {
    mockReview.mockResolvedValue({
      status: "stale_hld_source_bundle_payload",
      artifact: ARTIFACT_SUMMARY,
    ...
    const body = await res.json();
    expect(body.code).toBe("hld_source_bundle_payload_stale");
    expect(body.staleCode).toBe("recomputed_bundle_mismatch");
...
describe("POST .../hld-source-bundle/review - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    const secret = "boom-internal-stack-bundle-review";
    ...
    const body = await res.json();
    expect(body.code).toBe("hld_source_bundle_review_failed");
    expect(body.error).toBe("Unable to review HLD source bundle.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../hld-source-bundle/review - route surface", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-source-bundle/review/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-hld-source-bundle-approval-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the HLD source-bundle approval service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-hld-source-bundle-approval",
    ]);
  });
