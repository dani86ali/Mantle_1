...
// Mock auth and the normalization wrapper service so the route's auth-gate,
...
vi.mock("@/lib/projects/project-quick-bom-normalization", () => ({
  normalizeProjectQuickBomBoqFile: mockNormalize,
}));

import { POST } from "@/app/api/projects/[id]/quick-bom/files/[fileId]/normalize/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/files/[fileId]/normalize/route";
...
describe("POST .../quick-bom/files/[fileId]/normalize - auth", () => {
...
describe("POST .../quick-bom/files/[fileId]/normalize - tenant/param authority", () => {
...
describe("POST .../quick-bom/files/[fileId]/normalize - result mapping", () => {
...
describe("POST .../quick-bom/files/[fileId]/normalize - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_boq_normalization_failed");
    expect(body.error).toBe("Unable to normalize Quick BoM BoQ file.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../quick-bom/files/[fileId]/normalize - route surface", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-quick-bom-normalization-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the normalization wrapper service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-normalization",
    ]);
  });

  it("does not import DB, stores, raw BoQ loader/parser, artifact/approval/evidence stores, pricing, config expansion, mantle/export, runner, AI, catalog, engine, coordinator, or adapter modules", () => {
