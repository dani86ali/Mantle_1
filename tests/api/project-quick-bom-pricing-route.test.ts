...
// Mock auth and the Quick BoM pricing wrapper service so the route's auth-gate,
...
vi.mock("@/lib/projects/project-quick-bom-pricing", () => ({
  createProjectQuickBomPricedBoq: mockCreatePricedBoq,
}));

import { POST } from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/priced-boq/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/priced-boq/route";
...
describe("POST .../priced-boq - tenant/param authority", () => {
...
describe("POST .../priced-boq - result mapping", () => {
...
describe("POST .../priced-boq - service failure", () => {
  it("maps an unexpected service error to a controlled 500 distinct from the demo-fixture 500, without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_pricing_failed");
    expect(body.code).not.toBe("invalid_demo_pricing_fixture");
    expect(body.error).toBe("Unable to create Quick BoM priced BoQ.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../priced-boq - route surface", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(process.cwd(), "tests/api/project-quick-bom-pricing-route.test.ts");
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the pricing wrapper service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-pricing",
    ]);
  });

  it("does not import DB, stores, the pricing service/helpers, export, runner, AI, catalog, engine, coordinator, or adapter modules", () => {
