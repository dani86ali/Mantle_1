...
// Mock auth and the RFP SKU-resolution wrapper service so the route's auth-gate,
...
vi.mock("@/lib/projects/project-rfp-sku-resolution", () => ({
  createProjectRfpSkuResolutionDraft: mockCreateDraft,
}));

import { POST } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/sku-resolution/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/sku-resolution/route";
...
describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - auth", () => {
...
describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - tenant/param authority", () => {
...
describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - catalog profile fails closed", () => {
...
describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - result mapping", () => {
...
describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("rfp_sku_resolution_failed");
    expect(body.error).toBe("Unable to create RFP SKU resolution draft.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../rfp/artifacts/[artifactId]/sku-resolution - route surface", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/sku-resolution/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-sku-resolution-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the RFP SKU-resolution wrapper service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-sku-resolution",
    ]);
  });
