...
vi.mock("@/lib/projects/project-quick-bom-config-expansion-draft", () => ({
  createProjectQuickBomConfigurationExpansionDraft: mockCreateDraft,
}));

import { POST } from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/configuration-expansion/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/configuration-expansion/route";
...
describe("POST .../quick-bom/artifacts/[artifactId]/configuration-expansion - auth", () => {
...
describe("POST .../quick-bom/artifacts/[artifactId]/configuration-expansion - tenant/param authority", () => {
...
describe("POST .../quick-bom/artifacts/[artifactId]/configuration-expansion - result mapping", () => {
...
describe("POST .../quick-bom/artifacts/[artifactId]/configuration-expansion - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_configuration_expansion_draft_failed");
    expect(body.error).toBe(
      "Unable to create Quick BoM configuration expansion draft."
    );
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../quick-bom/artifacts/[artifactId]/configuration-expansion - route surface", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-quick-bom-config-expansion-route.test.ts"
  );
  ...
  it("imports only Next.js server primitives, requireAuth, and the draft wrapper service", () => {
    ...
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-config-expansion-draft",
    ]);
  });
