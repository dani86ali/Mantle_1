...
// Mock auth and the deterministic document-model review service so the route's auth
// gate, strict body parse, decision mapping, tenant/user/param authority, and result
// mapping are tested independent of the DB and any store.
...
vi.mock("@/lib/projects/project-rfp-hld-document-model-approval", () => ({
  reviewRfpHldDocumentModelArtifact: mockReview,
}));

import { POST } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-document-model/review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-document-model/review/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-docmodel-1";
const ARTIFACT = "art-docmodel-7";
const SESSION = {
  userId: "u-engineer",
...
describe("POST hld-document-model/review - auth", () => {
...
describe("POST hld-document-model/review - invalid body", () => {
  it("maps invalid JSON to 400 without calling the service", async () => {
    ...
    expect((await res.json()).code).toBe(
      "invalid_rfp_hld_document_model_review_request"
    );
    expect(mockReview).not.toHaveBeenCalled();
  });
...
describe("POST hld-document-model/review - authority and decision mapping", () => {
  ...
  it("reject with a note calls service with mapped 'rejected' and the trimmed note", async () => {
    const res = await POST(
      req({ decision: "reject", note: "  Needs an updated layout.  " }),
      PARAMS
    );
    ...
    expect(arg.note).toBe("Needs an updated layout.");
  });
...
describe("POST hld-document-model/review - result mapping", () => {
  ...
  it("maps artifact_not_found to 404 hld_document_model_artifact_not_found", async () => {
    ...
    const res = await POST(req(), PARAMS);
    expect(res.status).toBe(404);
    expect((await res.json()).code).toBe("hld_document_model_artifact_not_found");
  });

  it("maps artifact_not_hld_document_model to 409 with the artifact summary", async () => {
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("artifact_not_hld_document_model");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
  });

  it("maps artifact_not_reviewable to 409 hld_document_model_artifact_not_reviewable with the artifact summary", async () => {
    mockReview.mockResolvedValue({
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_document_model_artifact_not_reviewable");
    expect(body.artifact).toEqual(ARTIFACT_SUMMARY);
  });

  it("maps invalid_hld_document_model_payload to 409 hld_document_model_payload_invalid with the artifact summary", async () => {
    mockReview.mockResolvedValue({
  ...
  it("maps stale_hld_document_model_source_chain to 409 with staleCode and no raw messages/payload", async () => {
    ...
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_document_model_source_chain_stale");
  ...
  it("maps approval_failed to 409 hld_document_model_review_failed", async () => {
    mockReview.mockResolvedValue({ status: "approval_failed" });
    const res = await POST(req(), PARAMS);
    expect(res.status).toBe(409);
    expect((await res.json()).code).toBe("hld_document_model_review_failed");
  });
  ...
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("hld_document_model_review_failed");
    expect(body.error).toBe("Unable to review HLD document model.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("hld-document-model/review - route surface", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-document-model/review/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-hld-document-model-review-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports exactly next/server, requireAuth, and the document-model approval service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-hld-document-model-approval",
    ]);
  });
  ...
  it("touches no store/approval/inspection/raw-file/AI/pricing/sku/catalog/config/output module", () => {
