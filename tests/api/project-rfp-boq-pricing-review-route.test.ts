...
vi.mock("@/lib/projects/project-rfp-boq-pricing-review-workspace", () => ({
  loadRfpBoqPricedBoqReviewWorkspace: mockLoadReview,
}));

import { POST, GET } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/priced-boq/review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/priced-boq/review/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-pb-rfp-1";
const SESSION = {
  userId: "u-engineer",
...
const OK_RESULT = {
  ...
  workspace: { readiness: { projectId: PROJECT, ready: true } },
};
...
const REVIEW_WORKSPACE = {
  project: { id: PROJECT, tenantId: SESSION.tenantId, name: "Nesma RFP", mode: "rfp", createdAt: "2026-06-01T10:00:00.000Z", updatedAt: "2026-06-01T10:00:00.000Z" },
...
describe("POST .../rfp/.../priced-boq/review - auth", () => {
...
describe("POST .../rfp/.../priced-boq/review - body validation", () => {
...
describe("POST .../rfp/.../priced-boq/review - authority", () => {
...
describe("POST .../rfp/.../priced-boq/review - result mapping", () => {
  ...
  it("maps artifact_not_quick_bom to 409 artifact_not_priced_boq with the artifact summary and no quick_bom wording", async () => {
    ...
    expect(body.error).toBe("Artifact is not a priced_boq artifact.");
    expect(body.artifact).toEqual(artifact);
    // The internal service status name never leaks into the client-facing response.
    expect(JSON.stringify(body).toLowerCase()).not.toContain("quick");
  });
...
describe("POST .../rfp/.../priced-boq/review - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("priced_boq_review_failed");
    expect(body.error).toBe("Unable to review RFP priced BoQ.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("POST .../rfp/.../priced-boq/review - route surface", () => {
...
describe("GET .../rfp/.../priced-boq/review - auth", () => {
...
describe("GET .../rfp/.../priced-boq/review - authority", () => {
...
describe("GET .../rfp/.../priced-boq/review - result mapping", () => {
  ...
  it("maps wrong_mode to 409 with the RFP message", async () => {
    mockLoadReview.mockResolvedValue({ status: "wrong_mode" });
...
describe("GET .../rfp/.../priced-boq/review - loader failure", () => {
  it("maps an unexpected loader error to a safe 500 with code priced_boq_review_load_failed without exposing the thrown error", async () => {
    ...
    expect(body.error).toBe("Unable to load RFP priced BoQ review.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/priced-boq/review/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-boq-pricing-review-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, the RFP approval service, and the read-only RFP review loader", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-boq-approval",
      "@/lib/projects/project-rfp-boq-pricing-review-workspace",
    ]);
  });

  it("does not import DB, stores, pricing, export, config-expansion, runner, AI, catalog, engine, coordinator, adapter, quick-bom, or artifact-creation modules", () => {
