...
// Mock auth, the read-only review list inspection service, and the deterministic
// review-create service so the route's auth gate, body-ignoring on GET, scoped
// body parse on POST, tenant/user/param authority, and result mapping are tested
// independent of the DB.
const { mockRequireAuth, mockLoadList, mockCreateReview } = vi.hoisted(() => ({
...
vi.mock("@/lib/projects/project-rfp-hld-design-model-review-deterministic", () => ({
  createRfpHldDesignModelDeterministicReview: mockCreateReview,
}));
...
describe("POST /api/projects/[id]/rfp/hld-design-model-review - authority", () => {
  it("passes only session tenant/user, route project id, and the parsed source model id; decoys are ignored", async () => {
    const request = req({
      sourceHldDesignModelArtifactId: SOURCE_MODEL_ID,
...
describe("POST /api/projects/[id]/rfp/hld-design-model-review - result mapping", () => {
  ...
  it("maps source_bundle_unavailable to 409 with the blockerCode", async () => {
    mockCreateReview.mockResolvedValue({
      status: "source_bundle_unavailable",
      code: "source_bundle_not_resolved",
    });
    const res = await POST(req(), PARAMS);
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.code).toBe("hld_design_model_review_source_bundle_unavailable");
    expect(body.blockerCode).toBe("source_bundle_not_resolved");
  });

  it("maps invalid_review_payload to 409 hld_design_model_review_payload_invalid with errors", async () => {
    mockCreateReview.mockResolvedValue({
      status: "invalid_review_payload",
...
describe("route module purity (static source check)", () => {
  ...
  it("imports only Next.js server primitives, requireAuth, and the review services", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-hld-design-model-review-inspection",
      "@/lib/projects/project-rfp-hld-design-model-review-deterministic",
    ]);
  });
  ...
  it("touches no store directly and reads no file/evidence/pricing/sku/config/catalog/AI/approval modules", () => {
