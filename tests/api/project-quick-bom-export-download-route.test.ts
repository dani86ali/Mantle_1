...
// Mock auth and the Quick BoM export download service so the route's auth-gate,
// tenant/param authority, body/query-ignoring, byte streaming, and result-mapping are
// tested independent of the DB and the real filesystem.
...
vi.mock("@/lib/projects/project-quick-bom-export-download", () => ({
  loadProjectQuickBomExportDownload: mockLoadDownload,
}));

import { GET } from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/export-package/download/route";
import * as routeModule from "@/app/api/projects/[id]/quick-bom/artifacts/[artifactId]/export-package/download/route";
...
describe("GET .../export-package/download - auth", () => {
...
describe("GET .../export-package/download - tenant/param authority", () => {
...
describe("GET .../export-package/download - ok byte stream", () => {
...
describe("GET .../export-package/download - result mapping", () => {
...
describe("GET .../export-package/download - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("quick_bom_export_download_failed");
    expect(body.error).toBe("Unable to download Quick BoM export package.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("GET .../export-package/download - route surface", () => {
  it("exports GET only", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-quick-bom-export-download-route.test.ts"
  );
  const source = readFileSync(SRC_PATH, "utf8");

  it("imports only Next.js server primitives, requireAuth, and the download service", () => {
    const froms = Array.from(source.matchAll(/from\s+"([^"]+)"/g), (m) => m[1]);
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-quick-bom-export-download",
    ]);
  });

  it("does not import DB, stores, mantle/fixture/pricing/config, runner, AI, catalog, engine, coordinator, adapter, or artifact/approval creation modules", () => {
