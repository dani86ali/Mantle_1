...
// Mock auth and the read-only review detail inspection service so the route's
...
vi.mock("@/lib/projects/project-rfp-hld-design-model-review-inspection", () => ({
  loadRfpHldDesignModelReviewDetail: mockLoadDetail,
}));

import { GET } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-design-model-review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/hld-design-model-review/route";
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";

const PROJECT = "proj-rfp-1";
const ARTIFACT_ID = "art-hld-design-model-review-2";
const SESSION = {
  userId: "u-engineer",
...
const REVIEW = {
  ...
  sourceArtifactIds: ["hdm-1", "hsb-1"],
  sourceHldDesignModelArtifactId: "hdm-1",
  sourceHldSourceBundleArtifactId: "hsb-1",
  reviewedAt: "2026-06-20T12:00:00.000Z",
  reviewer: { type: "deterministic", id: SESSION.userId },
  sourceReferences: [
    { id: "ref-model", artifactId: "hdm-1" },
    { id: "ref-bundle", artifactId: "hsb-1" },
  ],
  findings: [],
...
describe("GET .../rfp/artifacts/[artifactId]/hld-design-model-review - auth", () => {
...
describe("GET .../hld-design-model-review (detail) - authority", () => {
...
describe("GET .../hld-design-model-review (detail) - result mapping", () => {
  ...
  it("maps artifact_not_found to 404 hld_design_model_review_artifact_not_found", async () => {
  ...
  it("maps artifact_not_hld_design_model_review to 409 with the lean artifact summary", async () => {
    const artifact = { ...ARTIFACT_SUMMARY, type: "hld_design_model" };
  ...
  it("maps invalid_payload to 409 hld_design_model_review_invalid_payload with the lean artifact summary", async () => {
  ...
  it("maps ok to 200 with { project, artifact, review }, no status discriminator, and no tenantId/storagePath leakage", async () => {
    const res = await GET(req(), PARAMS);
    expect(res.status).toBe(200);
...
describe("GET .../hld-design-model-review (detail) - route surface", () => {
  it("exports GET only", () => {
...
describe("route module purity (static source check)", () => {
  ...
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-hld-design-model-review-detail-route.test.ts"
  );
