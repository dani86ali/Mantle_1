...
// Mock auth and the RFP configuration-expansion review wrapper service + workspace
// loader so the route's auth-gate, body validation, tenant/user/param authority,
// body-ignoring, and result-mapping are tested independent of the DB and the
...
vi.mock("@/lib/projects/project-rfp-config-expansion-review", () => ({
  reviewProjectRfpConfigurationExpansionDraft: mockReview,
}));
vi.mock("@/lib/projects/project-rfp-config-expansion-review-workspace", () => ({
  loadRfpConfigurationExpansionReviewWorkspace: mockLoadWorkspace,
}));

import { POST, GET } from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/configuration-expansion/review/route";
import * as routeModule from "@/app/api/projects/[id]/rfp/artifacts/[artifactId]/configuration-expansion/review/route";
...
const OK_REVIEW = {
  project: { id: PROJECT, tenantId: SESSION.tenantId, name: "Test", mode: "rfp", createdAt: "2026-06-01T10:00:00.000Z", updatedAt: "2026-06-01T10:00:00.000Z" },
...
describe("POST .../rfp/.../configuration-expansion/review - auth", () => {
...
describe("POST .../rfp/.../configuration-expansion/review - request validation", () => {
...
describe("POST .../rfp/.../configuration-expansion/review - authority", () => {
...
describe("POST .../rfp/.../configuration-expansion/review - result mapping", () => {
...
describe("POST .../rfp/.../configuration-expansion/review - service failure", () => {
  it("maps an unexpected service error to a controlled 500 without exposing the thrown error", async () => {
    ...
    const body = await res.json();
    expect(body.code).toBe("rfp_configuration_expansion_review_failed");
    expect(body.error).toBe("Unable to review RFP configuration expansion draft.");
...
describe("GET .../rfp/.../configuration-expansion/review - auth", () => {
...
describe("GET .../rfp/.../configuration-expansion/review - authority", () => {
...
describe("GET .../rfp/.../configuration-expansion/review - result mapping", () => {
  ...
  it("maps wrong_mode to 409 with the RFP wrong-mode message", async () => {
    mockLoadWorkspace.mockResolvedValue({ status: "wrong_mode" });
...
describe("GET .../rfp/.../configuration-expansion/review - loader failure", () => {
  it("maps an unexpected loader error to a controlled 500 without exposing the thrown error", async () => {
    ...
    expect(res.status).toBe(500);
    const body = await res.json();
    expect(body.code).toBe("rfp_configuration_expansion_review_load_failed");
    expect(body.error).toBe("Unable to load RFP configuration expansion review.");
    expect(JSON.stringify(body)).not.toContain(secret);
  });
});

describe("RFP .../configuration-expansion/review - route surface", () => {
...
describe("route module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/app/api/projects/[id]/rfp/artifacts/[artifactId]/configuration-expansion/review/route.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/api/project-rfp-config-expansion-review-route.test.ts"
  );
  ...
  it("imports only Next.js server primitives, requireAuth, the wrapper service, and the review workspace loader", () => {
    ...
    expect(froms).toEqual([
      "next/server",
      "@/lib/middleware/auth",
      "@/lib/projects/project-rfp-config-expansion-review",
      "@/lib/projects/project-rfp-config-expansion-review-workspace",
    ]);
  });
