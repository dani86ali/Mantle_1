...
vi.mock(
  "@/lib/projects/project-rfp-extraction-delta-candidate-drafting-anthropic",
  () => ({
...
const SENTINEL_EXECUTOR = vi.fn(() => Promise.resolve({ candidates: [] }));

const FACTORY_ENV_KEYS = [
  "ANTHROPIC_API_KEY",
  "BOMATIC_RFP_EXTRACTION_DELTA_CANDIDATE_DRAFTING_MODEL",
  "BOMATIC_RFP_EXTRACTION_DELTA_CANDIDATE_DRAFTING_MAX_TOKENS",
];

describe("getConfiguredRfpExtractionDeltaCandidateDraftingExecutor", () => {
  ...
  it("returns null and never constructs the adapter when ANTHROPIC_API_KEY is missing", () => {
    expect(getConfiguredRfpExtractionDeltaCandidateDraftingExecutor()).toBeNull();
  ...
  it("constructs the adapter with the trimmed apiKey only and returns the mocked executor when no overrides are set", () => {
    ...
    const result = getConfiguredRfpExtractionDeltaCandidateDraftingExecutor();
...
describe("extraction-delta drafting executor factory module purity (static source check)", () => {
  const FACTORY_PATH = join(
    process.cwd(),
    "src/lib/projects/project-rfp-extraction-delta-candidate-drafting-executor.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-rfp-extraction-delta-candidate-drafting-executor.test.ts"
  );
  ...
  it("never imports @anthropic-ai/sdk or any other provider, AI, LLM, agent, adapter, coordinator, engine, catalog, pricing, SKU, config, export, Quick BoM, DB, store, route, or UI module", () => {
    for (const forbidden of [
      ...
      // the drafting-contract and adapter imports share this prefix).
