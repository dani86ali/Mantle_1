...
// DB and no real expansion/review run here; the wrapper is exercised in isolation.
...
// lean payload summary drops all lines, so none may surface in the wrapper response.
...
function makeServiceResult(
  ...
  return {
    ...
    // The wrapper reads only reviewResult.summary; the heavy accepted model is not needed.
    reviewResult: {
      summary: REVIEW_SUMMARY,
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - reviewedBy gate", () => {
  it("throws the exact reviewedBy message before any store call when reviewedBy is blank", async () => {
    await expect(
      reviewProjectQuickBomConfigurationExpansionDraft(input({ reviewedBy: "   " }))
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - project verification", () => {
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - draft artifact gates", () => {
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - known delegate error translation", () => {
  const CASES: Array<[string, ReviewProjectQuickBomConfigurationExpansionDraftResult]> = [
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - unexpected errors", () => {
  ...
  it("re-throws a non-Error rejection", async () => {
    ...
    await expect(reviewProjectQuickBomConfigurationExpansionDraft(input())).rejects.toBe(
      "plain string failure"
    );
  });
});

describe("reviewProjectQuickBomConfigurationExpansionDraft - ok summaries", () => {
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - immutability and copies", () => {
...
describe("module purity and surface (static source check)", () => {
  ...
  it("does not import the artifact-write helper, the draft service, approvals/evidence stores, the runner, pricing, export, AI, catalog, engine, coordinator, or adapter modules", () => {
