...
// The Quick BoM review service is now a thin wrapper over the shared, mode-gated
// review core. Mock ONLY the core: the wrapper's whole job is to pin
// expectedMode = "quick_bom", forward the rest of the input unchanged, and return the
// core's result verbatim. All deterministic behavior (store reads, draft gates,
// payload parsing, delegation, error translation, lean summaries) is proven in the
// core's own test; here we prove the wrapper adds nothing and hides nothing.
vi.mock("@/lib/projects/project-boq-config-expansion-review-core", () => ({
  reviewProjectBoqConfigurationExpansionDraftCore: vi.fn(),
}));

import * as serviceModule from "@/lib/projects/project-quick-bom-config-expansion-review";
import {
  reviewProjectQuickBomConfigurationExpansionDraft,
  type ReviewProjectQuickBomConfigurationExpansionDraftInput,
  type ReviewProjectQuickBomConfigurationExpansionDraftResult,
...
const OK_RESULT: ReviewProjectQuickBomConfigurationExpansionDraftResult = {
  status: "ok",
  artifact: {
    id: "art-ce-rev-9",
    projectId: PROJECT,
...
const WRONG_MODE_RESULT: ReviewProjectQuickBomConfigurationExpansionDraftResult = {
  status: "wrong_mode",
  project: {
    id: PROJECT,
    name: "RFP Bid",
...
function input(
  overrides: Partial<ReviewProjectQuickBomConfigurationExpansionDraftInput> = {}
): ReviewProjectQuickBomConfigurationExpansionDraftInput {
  return {
    tenantId: TENANT,
    projectId: PROJECT,
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - delegation", () => {
  it("calls the shared core exactly once with the input plus expectedMode quick_bom", async () => {
  ...
  it("forwards the decisions array by reference (no rewrap) and an empty array unchanged", async () => {
    ...
    coreMock.mockClear();
    await reviewProjectQuickBomConfigurationExpansionDraft(input({ decisions: [] }));
  ...
  it("never overrides expectedMode from a caller-supplied field on the input", async () => {
    // Even if a caller smuggles expectedMode onto the input, the wrapper pins quick_bom
    // last in the spread so the lane authority cannot be hijacked.
    const smuggled = input();
    (smuggled as Record<string, unknown>).expectedMode = "rfp";

    await reviewProjectQuickBomConfigurationExpansionDraft(smuggled);

    expect(coreMock.mock.calls[0][0].expectedMode).toBe("quick_bom");
  });
});

describe("reviewProjectQuickBomConfigurationExpansionDraft - result pass-through", () => {
  ...
  it("returns the core not_found result unchanged", async () => {
    const notFound: ReviewProjectQuickBomConfigurationExpansionDraftResult = { status: "not_found" };
  ...
  it("re-throws whatever the core throws (no swallow, no remap)", async () => {
    ...
    await expect(reviewProjectQuickBomConfigurationExpansionDraft(input())).rejects.toBe(boom);
  });
});

describe("reviewProjectQuickBomConfigurationExpansionDraft - immutability", () => {
  it("does not mutate the input object or its decisions (expectedMode is not added to the caller's input)", async () => {
    ...
    await reviewProjectQuickBomConfigurationExpansionDraft(inp);
...
describe("reviewProjectQuickBomConfigurationExpansionDraft - module purity and surface (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/lib/projects/project-quick-bom-config-expansion-review.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-quick-bom-config-expansion-review.test.ts"
  );
