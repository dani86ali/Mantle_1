...
// The RFP review-workspace loader is a thin wrapper over the shared, mode-gated
// review-workspace core. Mock ONLY the core: the wrapper's whole job is to pin
// expectedMode = "rfp", forward the positional tenant/project/artifact ids unchanged,
// and return the core's result verbatim. All deterministic behavior (store reads,
// artifact gates, payload parsing, the allowlist projection, the deterministic counts,
// the lean summaries) is proven in the core's own test; here we prove the wrapper adds
...
import * as workspaceModule from "@/lib/projects/project-rfp-config-expansion-review-workspace";
import {
  loadRfpConfigurationExpansionReviewWorkspace,
  type LoadRfpConfigExpansionReviewWorkspaceResult,
} from "@/lib/projects/project-rfp-config-expansion-review-workspace";
...
const PROJECT = "proj-rfp-ce-1";
const ARTIFACT_ID = "art-ce-draft-1";

const OK_RESULT: LoadRfpConfigExpansionReviewWorkspaceResult = {
  status: "ok",
  review: {
    mode: "draft",
    project: {
...
describe("loadRfpConfigurationExpansionReviewWorkspace - delegation", () => {
  it("calls the shared core exactly once with the positional ids plus expectedMode rfp", async () => {
    await loadRfpConfigurationExpansionReviewWorkspace(TENANT, PROJECT, ARTIFACT_ID);
  ...
  it("passes ONLY the tenant/project/artifact ids plus expectedMode - no other authority", async () => {
    ...
    expect(arg.expectedMode).toBe("rfp");
  });
});

describe("loadRfpConfigurationExpansionReviewWorkspace - result pass-through", () => {
  it("returns the core ok result unchanged (same reference)", async () => {
    coreMock.mockResolvedValue(OK_RESULT);

    const result = await loadRfpConfigurationExpansionReviewWorkspace(TENANT, PROJECT, ARTIFACT_ID);

    expect(result).toBe(OK_RESULT);
  });

  it("returns the core not_found result unchanged", async () => {
    const notFound: LoadRfpConfigExpansionReviewWorkspaceResult = { status: "not_found" };
  ...
  it("returns the core wrong_mode result unchanged", async () => {
    const wrongMode: LoadRfpConfigExpansionReviewWorkspaceResult = { status: "wrong_mode" };
  ...
  it("re-throws whatever the core throws (no swallow, no remap)", async () => {
    ...
    await expect(
      loadRfpConfigurationExpansionReviewWorkspace(TENANT, PROJECT, ARTIFACT_ID)
    ).rejects.toBe(boom);
  });
});

describe("loadRfpConfigurationExpansionReviewWorkspace - module purity and surface (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/lib/projects/project-rfp-config-expansion-review-workspace.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-rfp-config-expansion-review-workspace.test.ts"
  );
