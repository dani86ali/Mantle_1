...
import * as serviceModule from "@/lib/projects/project-rfp-sku-resolution";
import {
  createProjectRfpSkuResolutionDraft,
  type CreateProjectRfpSkuResolutionDraftInput,
} from "@/lib/projects/project-rfp-sku-resolution";
...
function input(): CreateProjectRfpSkuResolutionDraftInput {
  return {
    tenantId: TENANT,
    projectId: PROJECT,
...
describe("createProjectRfpSkuResolutionDraft - project verification", () => {
  it("returns not_found and does not load the artifact or create when the project is missing", async () => {
    getProjectMock.mockResolvedValue(null);

    const result = await createProjectRfpSkuResolutionDraft(input());
  ...
  it("returns a lean wrong_mode summary (no tenantId) and does not load the artifact or create for a non-rfp project", async () => {
  ...
  it("loads the project before loading the source artifact", async () => {
    await createProjectRfpSkuResolutionDraft(input());
...
describe("createProjectRfpSkuResolutionDraft - source artifact verification", () => {
...
describe("createProjectRfpSkuResolutionDraft - source artifact readiness", () => {
...
describe("createProjectRfpSkuResolutionDraft - known lower-level error translation", () => {
...
describe("createProjectRfpSkuResolutionDraft - unexpected errors", () => {
  it("re-throws an unexpected lower-level error unchanged", async () => {
    ...
    await expect(
      createProjectRfpSkuResolutionDraft(input())
    ).rejects.toBe(boom);
  });
...
describe("createProjectRfpSkuResolutionDraft - ok summaries", () => {
...
describe("createProjectRfpSkuResolutionDraft - immutability and copies", () => {
  it("does not mutate the input object", async () => {
    const inp = input();
    const snapshot = structuredClone(inp);

    await createProjectRfpSkuResolutionDraft(inp);
...
describe("createProjectRfpSkuResolutionDraft - delegation gating", () => {
...
describe("module purity and surface (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/lib/projects/project-rfp-sku-resolution.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-rfp-sku-resolution.test.ts"
  );
  ...
  it("does not import or broaden the Quick BoM wrapper it mirrors", () => {
    expect(source).not.toContain(
      'from "@/lib/projects/project-quick-bom-sku-resolution"'
    );
  });

  it("does not import the artifact-write helper, approval/evidence stores, the pure SKU/catalog helpers, the normalizer, raw BoQ loader, pricing, config expansion, mantle/export, runner, AI, catalog, coordinator, engine, or adapter modules", () => {
