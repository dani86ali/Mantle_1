...
import * as serviceModule from "@/lib/projects/project-rfp-boq-normalization";
...
describe("normalizeProjectRfpBoqFile - project verification", () => {
  ...
  it("returns a lean wrong_mode summary (no tenantId) and does not normalize for a non-rfp project", async () => {
...
describe("normalizeProjectRfpBoqFile - delegation", () => {
  it("passes tenantId, projectId, and fileId exactly to the normalizer", async () => {
    await normalizeProjectRfpBoqFile({
      tenantId: TENANT,
      projectId: PROJECT,
      fileId: FILE_ID,
    });
...
describe("normalizeProjectRfpBoqFile - known error translation", () => {
...
describe("normalizeProjectRfpBoqFile - unexpected errors", () => {
...
describe("normalizeProjectRfpBoqFile - ok summaries", () => {
...
describe("normalizeProjectRfpBoqFile - immutability and array copies", () => {
  it("does not mutate the input object", async () => {
    const input = { tenantId: TENANT, projectId: PROJECT, fileId: FILE_ID };
    const snapshot = structuredClone(input);

    await normalizeProjectRfpBoqFile(input);
...
describe("module purity and surface (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/lib/projects/project-rfp-boq-normalization.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-rfp-boq-normalization.test.ts"
  );
  ...
  it("does not import the artifact/approval/evidence/file stores, raw BoQ loader, pricing, config expansion, mantle/export, runner, AI, catalog, coordinator, engine, or adapter modules", () => {
