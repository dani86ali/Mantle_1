...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - error statuses", () => {
...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - mode gate", () => {
  ...
  it("returns wrong_mode for an rfp project when expectedMode is quick_bom, before any artifact load", async () => {
    mockGetProject.mockResolvedValue({ ...BASE_PROJECT, mode: "rfp" });
    const result = await loadCore({ expectedMode: "quick_bom" });
    expect(result.status).toBe("wrong_mode");
    expect(mockGetArtifact).not.toHaveBeenCalled();
  });
  ...
  it("proceeds past the mode gate for a quick_bom project when expectedMode is quick_bom", async () => {
    const result = await loadCore({ expectedMode: "quick_bom" });
    expect(result.status).toBe("ok");
    if (result.status !== "ok") return;
    expect(result.review.project.mode).toBe("quick_bom");
  });
});

describe("loadProjectBoqPricedBoqReviewWorkspaceCore - ok projection", () => {
  ...
  it("scopes both store calls to a non-default tenant/project/artifact when overridden", async () => {
    await loadCore({
      tenantId: "tenant-2",
      projectId: "proj-2",
      artifactId: "art-2",
    });
    expect(mockGetProject).toHaveBeenCalledWith("tenant-2", "proj-2", {
      includeArchived: true,
    });
    expect(mockGetArtifact).toHaveBeenCalledWith("tenant-2", "proj-2", "art-2");
  });
...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - reviewSummary counts", () => {
  it("counts pricedLineCount from line statuses, not from payload summary", async () => {
    // Both lines have status priced in this override; the payload summary still says 1.
    const twoPriced = {
      ...BASE_PAYLOAD,
      lines: [
    ...
    mockGetArtifact.mockResolvedValue({ ...BASE_ARTIFACT, payload: twoPriced });
    const result = await loadCore();
    if (result.status !== "ok") throw new Error("expected ok");
    ...
    // The trusted payload summary count is NOT what the deterministic recount returns.
    expect(result.review.payloadSummary.pricingSummary.pricedLineCount).toBe(1);
  });
...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - array copying", () => {
...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - field canary checks", () => {
...
describe("loadProjectBoqPricedBoqReviewWorkspaceCore - static source purity", () => {
  const SRC = join(process.cwd(), "src/lib/projects/project-boq-pricing-review-workspace-core.ts");
  const TEST = join(process.cwd(), "tests/lib/projects/project-boq-pricing-review-workspace-core.test.ts");
