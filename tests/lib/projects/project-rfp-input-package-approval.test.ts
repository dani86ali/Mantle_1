...
// reviewability gate is a true integration check, mirroring the Quick BoM
...
import {
  reviewRfpInputPackageArtifact,
  type ReviewRfpInputPackageArtifactInput,
  type ReviewRfpInputPackageArtifactResult,
...
function review(
  overrides: Partial<ReviewRfpInputPackageArtifactInput> = {}
): Promise<ReviewRfpInputPackageArtifactResult> {
  return reviewRfpInputPackageArtifact({
    tenantId: TENANT,
    projectId: PROJECT,
    artifactId: ARTIFACT,
    decision: "approved",
...
describe("reviewRfpInputPackageArtifact - input validation", () => {
...
describe("reviewRfpInputPackageArtifact - project gates", () => {
...
describe("reviewRfpInputPackageArtifact - artifact gates", () => {
  ...
  it("rejects every non-input_package type as artifact_not_input_package without creating an approval", async () => {
    ...
    for (const type of otherTypes) {
      ...
      // Right stage and a reviewable status prove the TYPE gate fires first.
  ...
  it("rejects an input_package artifact outside intake_package_review as artifact_not_input_package without creating an approval", async () => {
...
describe("reviewRfpInputPackageArtifact - approval", () => {
...
describe("reviewRfpInputPackageArtifact - ok", () => {
...
describe("reviewRfpInputPackageArtifact - immutability", () => {
  it("does not mutate the input or the loaded artifact", async () => {
    ...
    await reviewRfpInputPackageArtifact(input);
...
describe("module purity (static source check)", () => {
  const SRC_PATH = join(
    process.cwd(),
    "src/lib/projects/project-rfp-input-package-approval.ts"
  );
  const TEST_PATH = join(
    process.cwd(),
    "tests/lib/projects/project-rfp-input-package-approval.test.ts"
  );
  ...
  it("imports no filesystem, file store, draft service, parser/loader, evidence, pricing, config-expansion, export, runner, AI, catalog, coordinator, engine, or adapter module", () => {
