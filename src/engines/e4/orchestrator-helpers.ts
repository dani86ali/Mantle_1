/**
 * E4 orchestrator helpers — shared step-logging utilities for phase 1 / phase 2.
 * Keeps the orchestrator under the 200-line cap from CLAUDE.md.
 */

import { logEntry } from '@/coordinator/logger';
import type { EngineInput } from '@/coordinator/types';

export interface E4StepLog {
  step: number;
  name: string;
...
export async function runStep<T>(
  ...
  const entry: E4StepLog = { step, name, status: 'started', durationMs: 0 };
  logs.push(entry);
  try {
...
export function recordSkip(step: number, name: string, logs: E4StepLog[]): void {
