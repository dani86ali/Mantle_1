 ...
 * no-expansion draft may legitimately require no decisions); any tenantId/projectId/
 * artifactId/configurationExpansionDraftArtifactId/reviewedBy/reviewedAt in the body
 * (or in individual decisions) is ignored. A missing/malformed body or any invalid
 * decision yields 400 invalid_configuration_expansion_review_request. The service
 * result maps to HTTP: not_found -> 404, wrong_mode -> 409 (lean project),
...
export async function POST(
  ...
  try {
    const result = await reviewProjectQuickBomConfigurationExpansionDraft({
