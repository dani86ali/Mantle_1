/**
 * POST /api/projects/[id]/quick-bom/artifacts/[artifactId]/export-package - create a
 * Mantle `export_package` artifact (status needs_review) from one already-approved
 * `priced_boq` artifact. This is export package CREATION only - not download/serve
 * (Prompt 95) and not export approval. The workbook output path is generated
 * server-side by the wrapper service.
 *
 ...
 * (quick_bom_export_package_failed) that never exposes the thrown error. Imports only
 * Next.js server primitives, requireAuth, and the Quick BoM export wrapper service.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { createProjectQuickBomExportPackage } from "@/lib/projects/project-quick-bom-export";
...
export async function POST(
  ...
  try {
    const result = await createProjectQuickBomExportPackage({
    ...
    return NextResponse.json(
      {
        code: "quick_bom_export_package_failed",
        error: "Unable to create Quick BoM export package.",
      },
      { status: 500 }
    );
  }
