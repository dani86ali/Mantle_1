/**
 * POST /api/projects/[id]/quick-bom/artifacts/[artifactId]/priced-boq
 * - create a `priced_boq` DRAFT (status needs_review) from one already-approved,
 * reviewed configuration_expansion artifact, priced deterministically against the
 ...
 * unexpected service error maps to a controlled 500 (quick_bom_pricing_failed) that
 * never exposes the thrown error. Imports only Next.js server primitives, requireAuth,
 * and the Quick BoM pricing wrapper service.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { createProjectQuickBomPricedBoq } from "@/lib/projects/project-quick-bom-pricing";
...
export async function POST(
  ...
  try {
    const result = await createProjectQuickBomPricedBoq({
    ...
    return NextResponse.json(
      {
        code: "quick_bom_pricing_failed",
        error: "Unable to create Quick BoM priced BoQ.",
      },
      { status: 500 }
    );
  }
