/**
 * POST /api/projects/[id]/rfp/boq/approvals - record an approve/reject
 * decision for an EXACT RFP BoQ artifact version.
 *
 ...
 * or an invalid body maps to 400. Maps the RFP BoQ service's discriminated result
 * to HTTP and never exposes a thrown error. Imports only Next.js server
 * primitives, requireAuth, and the RFP BoQ review service (no DB/AI/catalog/
 * engine/coordinator); it never prices, exports, or expands configuration.
 */
...
function invalidRequest(): NextResponse {
  return NextResponse.json(
    {
      code: "invalid_project_rfp_boq_approval_request",
...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: "Artifact is not approval-gated for RFP BoQ.",
    ...
    return NextResponse.json(
      {
        code: "rfp_boq_approval_failed",
        error: "Unable to record RFP BoQ approval.",
      },
      { status: 500 }
    );
  }
