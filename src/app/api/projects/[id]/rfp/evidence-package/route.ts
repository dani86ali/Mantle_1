/**
 * GET /api/projects/[id]/rfp/evidence-package.
 *
 * Read-only list of one rfp Project's evidence_package artifact versions as
 * lean serializable summaries (identifiers, ISO dates, counts, copied source
 * arrays - never an evidence text body, never a persisted table row, never a
 * tenantId or a storage path). This route parses no documents, reads no file
 * bytes, writes nothing, drafts/reviews/applies/approves no delta, drafts or
 * approves no evidence package, generates no requirements/compliance/HLD/
 * proposal, prices nothing, resolves no SKU/configuration, exports nothing,
 ...
 * (rfp_evidence_package_inspection_failed) that never exposes the thrown
...
import { loadRfpEvidencePackageList } from "@/lib/projects/project-rfp-evidence-package-inspection";
...
export async function GET(
  ...
  try {
    const result = await loadRfpEvidencePackageList({
