/**
 * POST /api/projects/[id]/rfp/artifacts/[artifactId]/configuration-expansion
...
import { createProjectRfpConfigurationExpansionDraft } from "@/lib/projects/project-rfp-config-expansion-draft";
...
export async function POST(
  ...
  try {
    const result = await createProjectRfpConfigurationExpansionDraft({
    ...
    return NextResponse.json(
      {
        code: "rfp_configuration_expansion_draft_failed",
        error: "Unable to create RFP configuration expansion draft.",
      },
      { status: 500 }
    );
  }
