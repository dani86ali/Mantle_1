/**
 * GET /api/projects/[id]/rfp/artifacts/[artifactId]/export-package/download -
 * stream the workbook of one already-approved RFP `export_package` artifact. This is
 * download/serve ONLY: it never builds, regenerates, re-prices, re-resolves SKUs,
 * re-expands configuration, or approves anything.
 *
 ...
 * error maps to a controlled 500 (rfp_boq_export_download_failed) that never exposes the
 * thrown error. Imports only Next.js server primitives, requireAuth, and the RFP BoQ
 * export download service.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { loadProjectRfpBoqExportDownload } from "@/lib/projects/project-rfp-boq-export-download";
...
export async function GET(
  ...
  try {
    const result = await loadProjectRfpBoqExportDownload({
    ...
    return NextResponse.json(
      {
        code: "rfp_boq_export_download_failed",
        error: "Unable to download RFP BoQ export package.",
      },
      { status: 500 }
    );
  }
