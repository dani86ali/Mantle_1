/**
 * GET + POST /api/projects/[id]/rfp/artifacts/[artifactId]/sku-resolution/review.
 *
 ...
 * RFP SKU review wrapper service, and the read-only review workspace loader (no
...
import {
  ...
  type RfpSkuResolutionReviewActionInput,
...
function parseReviewActions(
  body: unknown
): RfpSkuResolutionReviewActionInput[] | null {
  if (!isRecord(body) || !Array.isArray(body.actions) || body.actions.length === 0) {
    return null;
  }

  const parsed: RfpSkuResolutionReviewActionInput[] = [];
...
export async function POST(
  ...
  try {
    const result = await reviewProjectRfpSkuResolutionLines({
    ...
    return NextResponse.json(
      {
        code: "rfp_sku_resolution_review_failed",
        error: "Unable to review RFP SKU resolution lines.",
      },
      { status: 500 }
    );
  }
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      {
        code: "rfp_sku_resolution_review_load_failed",
        error: "Unable to load RFP SKU resolution review.",
      },
      { status: 500 }
    );
  }
