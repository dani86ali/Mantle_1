/**
 * POST /api/projects/[id]/rfp/artifacts/[artifactId]/hld-source-bundle/review.
 *
 * Records an approve/reject decision for the EXACT hld_source_bundle artifact named
 * in the URL (the HLD source-bundle review gate). This route never reads file
 ...
 * invalid decision yields 400 invalid_rfp_hld_source_bundle_review_request. Result
 ...
 * hld_source_bundle_artifact_not_found, artifact_not_hld_source_bundle -> 409
 * artifact_not_hld_source_bundle (with the artifact summary), artifact_not_reviewable
 * -> 409 hld_source_bundle_artifact_not_reviewable (with the artifact summary),
 * invalid_hld_source_bundle_payload -> 409 hld_source_bundle_payload_invalid (with
 * the artifact summary), stale_hld_source_bundle_payload -> 409
 * hld_source_bundle_payload_stale (with the artifact summary, the staleCode, and any
 * messages/errors), approval_failed -> 409 hld_source_bundle_review_failed, ok -> 200
 ...
 * Next.js server primitives, requireAuth, and the HLD source-bundle approval service.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { reviewRfpHldSourceBundleArtifact } from "@/lib/projects/project-rfp-hld-source-bundle-approval";
...
function invalidRequest(): NextResponse {
  return NextResponse.json(
    {
      code: "invalid_rfp_hld_source_bundle_review_request",
...
export async function POST(
  ...
  try {
    const result = await reviewRfpHldSourceBundleArtifact({
      ...
      return NextResponse.json(
        {
          ...
          error: "Artifact is not a reviewable hld_source_bundle artifact.",
    ...
    if (result.status === "artifact_not_reviewable") {
      return NextResponse.json(
        {
          code: "hld_source_bundle_artifact_not_reviewable",
          error: "HLD source bundle artifact is not reviewable.",
          artifact: result.artifact,
        },
        { status: 409 }
      );
    }
    if (result.status === "invalid_hld_source_bundle_payload") {
      return NextResponse.json(
        {
          code: "hld_source_bundle_payload_invalid",
    ...
    if (result.status === "stale_hld_source_bundle_payload") {
      return NextResponse.json(
        {
          code: "hld_source_bundle_payload_stale",
          error: "HLD source bundle payload is stale; recompile before approval.",
          artifact: result.artifact,
    ...
    if (result.status === "approval_failed") {
      return NextResponse.json(
        {
          code: "hld_source_bundle_review_failed",
          error: "HLD source bundle review could not be recorded.",
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        code: "hld_source_bundle_review_failed",
        error: "Unable to review HLD source bundle.",
      },
      { status: 500 }
    );
  }
