/**
 * GET /api/projects/[id]/rfp/artifacts/[artifactId]/hld-diagram.
 *
 * Read-only sanitized detail of the EXACT internal `hld_diagram` draft artifact
 ...
 * generation, calls no AI, and emits no final HLD document/HTML/draw.io/
 * diagram-markup/proposal/export - it only invokes the read-only inspection
 ...
 * summary), artifact_not_found -> 404 hld_diagram_artifact_not_found,
 * artifact_not_hld_diagram -> 409 artifact_not_hld_diagram (with the lean
 * payload-free artifact summary), invalid_payload -> 409
 * hld_diagram_invalid_payload (with the same lean summary), ok -> 200 with
 * { project, artifact, diagram }. An unexpected service error maps to a controlled
 * 500 that never exposes the thrown error. Imports only Next.js server primitives,
 * requireAuth, and the inspection service.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { loadRfpHldDiagramDetail } from "@/lib/projects/project-rfp-hld-diagram-inspection";
...
export async function GET(
  ...
  try {
    const result = await loadRfpHldDiagramDetail({
      ...
      return NextResponse.json(
        {
          ...
          error: "Artifact is not an hld_diagram artifact.",
    ...
    if (result.status === "invalid_payload") {
      return NextResponse.json(
        {
          code: "hld_diagram_invalid_payload",
    ...
    return NextResponse.json(
      {
        ...
        error: "Unable to inspect HLD diagram draft.",
      },
      { status: 500 }
    );
  }
