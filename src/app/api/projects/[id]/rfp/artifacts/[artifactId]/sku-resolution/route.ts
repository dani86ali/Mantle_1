/**
 * POST /api/projects/[id]/rfp/artifacts/[artifactId]/sku-resolution -
 * create a sku_resolution draft from an already-recorded RFP normalized_boq
 * artifact.
 *
 ...
 * server primitives, requireAuth, and the RFP SKU-resolution wrapper service (no
 * DB, stores, raw BoQ loader/parser, artifact/approval/evidence store, pricing,
 * config expansion, export, runner, AI, catalog, engine, coordinator, or
 * adapter).
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { createProjectRfpSkuResolutionDraft } from "@/lib/projects/project-rfp-sku-resolution";
...
export async function POST(
      ...
      return NextResponse.json(
        {
          ...
          error:
            "catalogProfile is not supported for RFP BoQ SKU resolution; the default Quick BoM catalog is always used.",
        },
        { status: 400 }
      );
    }
  ...
  try {
    const result = await createProjectRfpSkuResolutionDraft({
    ...
    return NextResponse.json(
      {
        code: "rfp_sku_resolution_failed",
        error: "Unable to create RFP SKU resolution draft.",
      },
      { status: 500 }
    );
  }
