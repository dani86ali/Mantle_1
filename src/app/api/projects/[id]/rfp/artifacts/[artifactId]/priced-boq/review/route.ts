/**
 * GET + POST /api/projects/[id]/rfp/artifacts/[artifactId]/priced-boq/review.
 *
 * GET returns a read-only priced-BoQ line-review projection of the EXACT `priced_boq`
 * artifact named in the URL via the RFP read model loader, tenant-scoped on
 ...
 * the URL (the Pricing Review Approval step after the RFP priced BoQ draft is created
 * needs_review). This route never prices, exports, expands configuration, looks up a
 * catalog, runs the runner, or calls AI - it only records one human decision.
 *
 ...
 * are the only project / priced_boq artifact authority. The request body supplies ONLY
 * { decision, note? }; any tenantId/projectId/artifactId/pricedBoqArtifactId/decidedBy/
 * decidedAt/pricingConfig/unitListPriceSarBySku/exportPath/allowedArtifactTypes in the
 * body is ignored. A missing/malformed body or an invalid decision yields 400
 * invalid_priced_boq_review_request. The shared exact-artifact approval service is
 * called with allowedArtifactTypes ["priced_boq"], so it can only ever approve/reject a
 * priced_boq artifact - never a sku_resolution, configuration_expansion, or
 * export_package artifact. The service result maps to HTTP: not_found -> 404
 * project_not_found, wrong_mode -> 409 wrong_project_mode (with the lean project
 * summary), artifact_not_found -> 404 priced_boq_artifact_not_found,
 * artifact_not_quick_bom -> 409 artifact_not_priced_boq (with the artifact summary, no
 * quick_bom wording), artifact_not_reviewable -> 409
 * priced_boq_artifact_not_reviewable (with the artifact summary), approval_failed -> 409
 * priced_boq_review_failed, ok -> 200 with { approval, artifactStatus, stageStatus,
 * workspace }. An unexpected service error maps to a controlled 500
 * (priced_boq_review_failed) that never exposes the thrown error. Imports only Next.js
 * server primitives, requireAuth, the exact-artifact approval service, and the
 * read-only priced-BoQ review workspace loader.
 */
import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/lib/middleware/auth";
import { reviewProjectRfpBoqArtifact } from "@/lib/projects/project-rfp-boq-approval";
...
export async function POST(
  ...
  try {
    const result = await reviewProjectRfpBoqArtifact({
    ...
    return NextResponse.json(
      {
        code: "priced_boq_review_failed",
        error: "Unable to review RFP priced BoQ.",
      },
      { status: 500 }
    );
  }
...
export async function GET(
  ...
  try {
    const result = await loadRfpBoqPricedBoqReviewWorkspace(
      session.tenantId,
      params.id,
      params.artifactId
    );
    ...
    return NextResponse.json(
      {
        ...
        error: "Unable to load RFP priced BoQ review.",
      },
      { status: 500 }
    );
  }
