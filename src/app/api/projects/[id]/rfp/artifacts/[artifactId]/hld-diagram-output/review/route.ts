/**
 * POST /api/projects/[id]/rfp/artifacts/[artifactId]/hld-diagram-output/review.
 *
 * Records one human approve/reject decision for the EXACT internal `hld_diagram_output`
 * artifact named in the URL (the HLD diagram-output review gate); it only records one
 ...
 * invalid_rfp_hld_diagram_output_review_request. A present note is trimmed and omitted
 * when blank.
 *
 ...
 * artifact_not_hld_diagram_output (with the artifact summary), artifact_not_reviewable
 * -> 409 hld_diagram_output_artifact_not_reviewable (with the artifact summary),
 * invalid_hld_diagram_output_payload -> 409 hld_diagram_output_payload_invalid (with the
 * artifact summary), stale_hld_diagram_output_source_chain -> 409
 * hld_diagram_output_source_chain_stale (with the artifact summary and the stable
 * staleCode, never raw messages/payload), approval_failed -> 409
 * hld_diagram_output_review_failed, ok -> 200 with { approval, artifactStatus,
 ...
 * and the HLD diagram-output approval service.
 */
...
function invalidRequest(): NextResponse {
  return NextResponse.json(
    {
      code: "invalid_rfp_hld_diagram_output_review_request",
 ...
 * optional "note"; any extra key is rejected. The client decision "approve"/"reject" is
 * mapped to the canonical "approved"/"rejected". A present note must be a string and is
 * trimmed; a blank trimmed note is omitted.
 */
...
export async function POST(
  ...
  try {
    const result = await reviewRfpHldDiagramOutputArtifact({
      ...
      return NextResponse.json(
        {
          ...
          error: "Artifact is not a reviewable hld_diagram_output artifact.",
    ...
    if (result.status === "artifact_not_reviewable") {
      return NextResponse.json(
        {
          code: "hld_diagram_output_artifact_not_reviewable",
          error: "HLD diagram output artifact is not reviewable.",
          artifact: result.artifact,
        },
        { status: 409 }
      );
    }
    if (result.status === "invalid_hld_diagram_output_payload") {
      return NextResponse.json(
        {
    ...
    if (result.status === "stale_hld_diagram_output_source_chain") {
      return NextResponse.json(
        {
          code: "hld_diagram_output_source_chain_stale",
          error:
            "HLD diagram output source chain is stale; regenerate before approval.",
          artifact: result.artifact,
    ...
    if (result.status === "approval_failed") {
      return NextResponse.json(
        {
          code: "hld_diagram_output_review_failed",
          error: "HLD diagram output review could not be recorded.",
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        code: "hld_diagram_output_review_failed",
        error: "Unable to review HLD diagram output.",
      },
      { status: 500 }
    );
  }
