/**
 * Shared Project BoQ pricing core: create a `priced_boq` DRAFT artifact (status
 ...
 *
 * Scope is the single product step "approved configuration_expansion -> priced_boq
 * draft", shared by every BoQ pricing lane (Quick BoM, RFP). It verifies the Project
 * (not_found) and gates project.mode against an INJECTED expectedMode (wrong_mode) so
 * each lane pins its own mode without forking this logic, then requires the
 * Project-owned `pricingConfig` to exist (pricing_config_missing) BEFORE loading any
 * artifact or pricing, then delegates the load+price+persist to the existing
 ...
 * request bodies supply no price map, pricingConfig, or approver; lane wrappers and
 * routes are responsible for sourcing tenant/project/artifact identifiers from
 * session and route authority, while pricing authority comes only from the Project
 * and committed fixture.
 *
 ...
 * helpers it composes. They are private to those modules, so this core mirrors the
...
/** Full core input: the request plus the mode a lane wrapper pins. */
export interface CreateProjectBoqPricedBoqCoreInput
  extends CreateProjectBoqPricedBoqRequest {
...
export interface ProjectBoqPricingProjectSummary {
  id: string;
  name: string;
  customerName?: string;
  mode: ProjectMode;
...
export interface ProjectBoqPricingArtifactSummary {
  id: string;
  projectId: string;
 ...
 * never surfaced.
 */
export interface ProjectBoqPricingPayloadSummary {
  ...
  pricingSource: ProjectBoqPricingSourceSummary;
  ...
  lineCount: number;
  summary: ProjectBoqPricingSummary;
}

/** Discriminated result of {@link createProjectBoqPricedBoq}. */
export type CreateProjectBoqPricedBoqCoreResult =
  | { status: "not_found" }
  | { status: "wrong_mode"; project: ProjectBoqPricingProjectSummary }
  ...
  | {
      status: "ok";
      artifact: ProjectBoqPricingArtifactSummary;
      payloadSummary: ProjectBoqPricingPayloadSummary;
      pricingSummary: ProjectBoqPricingSummary;
    };

/** Lean wrong-mode project projection; tenantId is never surfaced. */
function toProjectSummary(project: Project): ProjectBoqPricingProjectSummary {
  return {
    id: project.id,
    name: project.name,
...
function toArtifactSummary(artifact: ProjectArtifact): ProjectBoqPricingArtifactSummary {
  return {
    id: artifact.id,
...
function copyPricingSummary(summary: ProjectBoqPricingSummary): ProjectBoqPricingSummary {
 ...
 */
function toPayloadSummary(payload: PricedBoqArtifactPayload): ProjectBoqPricingPayloadSummary {
  return {
...
function translateDelegateError(
  message: string
): CreateProjectBoqPricedBoqCoreResult | null {
...
/**
 * Price one already-approved, reviewed `configuration_expansion` artifact into a new
 * `needs_review` `priced_boq` artifact, tenant-scoped on every store/service call. It
 * verifies the Project within its tenant (not_found), gates project.mode against the
 * injected expectedMode (wrong_mode, lean summary), then requires the Project-owned
 ...
 * The input, Project, pricingConfig, fixture map, and delegate result are never
 * mutated.
 */
export async function createProjectBoqPricedBoq(
  input: CreateProjectBoqPricedBoqCoreInput
): Promise<CreateProjectBoqPricedBoqCoreResult> {
  const { tenantId, projectId, configurationExpansionArtifactId, expectedMode } = input;
