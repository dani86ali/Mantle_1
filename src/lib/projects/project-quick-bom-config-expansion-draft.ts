/**
 * Quick BoM configuration-expansion DRAFT service: turn one already-APPROVED
 ...
 *
 * Scope (Prompt 90) is the single product step "approved sku_resolution ->
 * configuration_expansion draft". It verifies the Project (not_found /
 * wrong_mode), the exact source `sku_resolution` artifact (existence / type /
 * approved status / payload shape), and the referenced `normalized_boq` artifact
 * (existence / type / version match / readiness / payload shape), then builds the
 * draft deterministically from the existing pure builder and the approved
 * Honeywell rule pack and persists exactly ONE new artifact via
 * createProjectArtifactVersion with status `needs_review`.
 *
...
/** Input for {@link createProjectQuickBomConfigurationExpansionDraft}. */
export interface CreateProjectQuickBomConfigurationExpansionDraftInput {
  tenantId: string;
  projectId: string;
...
export interface QuickBomConfigExpansionProjectSummary {
  id: string;
  name: string;
  customerName?: string;
  mode: ProjectMode;
...
export interface QuickBomConfigExpansionArtifactSummary {
  id: string;
  projectId: string;
...
export interface QuickBomConfigExpansionPayloadSummary {
...
export type CreateProjectQuickBomConfigurationExpansionDraftResult =
  | { status: "not_found" }
  | { status: "wrong_mode"; project: QuickBomConfigExpansionProjectSummary }
  | { status: "sku_resolution_not_found" }
  | {
      status: "artifact_not_sku_resolution";
      artifact: QuickBomConfigExpansionArtifactSummary;
    }
  | {
  ...
  | {
      status: "ok";
      artifact: QuickBomConfigExpansionArtifactSummary;
      payloadSummary: QuickBomConfigExpansionPayloadSummary;
    };
...
function toProjectSummary(
  project: Project
): QuickBomConfigExpansionProjectSummary {
  return {
    id: project.id,
    name: project.name,
...
): QuickBomConfigExpansionArtifactSummary {
  return {
    id: artifact.id,
 ...
 * `sku_resolution` artifact, tenant-scoped on every store call. Runs the Project
 * and source-artifact gates in the order declared on the result union; no gate
 * persists anything. On a passing chain it builds the draft deterministically from
 * the pure builder and the approved Honeywell rule pack and creates exactly one
 * `needs_review` `configuration_expansion` draft artifact, returning lean
...
export async function createProjectQuickBomConfigurationExpansionDraft(
  ...
  const { tenantId, projectId, skuResolutionArtifactId } = input;
