/**
 * Quick BoM export service: create a Mantle `export_package` artifact (status
 * `needs_review`) from one already-APPROVED `priced_boq` artifact. Source of truth:
 * C:\Pre-Sales\bomatic_planning\MVP_CANONICAL_PROJECT_STATE.md (section 10 Priced
 * Output Contract; section 19 task 8k). Canonical shapes: src/types/project.ts.
 *
 * Scope (Prompt 94) is the single product step "approved priced_boq -> export_package
 * creation". This is export package CREATION only - not download/serve (Prompt 95) and
 * not export approval. It verifies the Project (not_found / wrong_mode), generates a
 * server-side workbook output path, and delegates the load+map+write+persist to the
 * existing deterministic {@link createMantleExportArtifact}. That delegate reads exactly
 * one approved priced_boq artifact, builds the pure Mantle price-estimate model, writes
 * the workbook, and persists ONE `needs_review` `export_package` at `export_approval`.
 *
 ...
 * workbook bytes, the priced_boq payload, priced lines, amounts, or the category map),
 * and never mutates its input, the Project, the fixture map, or the delegate result.
 */
 ...
 * wrapper mirrors the literals deliberately to translate them into safe statuses
...
export type QuickBomExportType = MantleExportArtifactPayload["exportType"];

/** The Mantle total-bucket roll-up echoed onto the summaries (no per-line detail). */
export type QuickBomExportTotals = MantleExportArtifactPayload["totals"];
...
export interface CreateProjectQuickBomExportPackageInput {
  tenantId: string;
  projectId: string;
...
export interface QuickBomExportProjectSummary {
  id: string;
  name: string;
  customerName?: string;
  mode: ProjectMode;
...
export interface QuickBomExportArtifactSummary {
  id: string;
  projectId: string;
...
export type QuickBomExportCategorySourceSummary = typeof CATEGORY_SOURCE;
 ...
 * never surfaced.
 */
export interface QuickBomExportPayloadSummary {
  exportType: QuickBomExportType;
  ...
  rowCount: number;
  totals: QuickBomExportTotals;
  warnings: string[];
  categorySource: QuickBomExportCategorySourceSummary;
}

/** Lean export roll-up: row count, totals, warnings, and the generated workbook path. */
export interface QuickBomExportSummary {
  rowCount: number;
  totals: QuickBomExportTotals;
  warnings: string[];
  workbookFilePath: string;
...
export type CreateProjectQuickBomExportPackageResult =
  | { status: "not_found" }
  | { status: "wrong_mode"; project: QuickBomExportProjectSummary }
  ...
  | {
      status: "ok";
      artifact: QuickBomExportArtifactSummary;
      payloadSummary: QuickBomExportPayloadSummary;
      exportSummary: QuickBomExportSummary;
    };
 ...
 * never derived from caller input or a request body; project/priced-boq ids are only
 * sanitized for readability and a randomUUID() guarantees uniqueness. tmpdir() already
 * exists, so no directory is created.
 */
function generateOutputPath(projectId: string, pricedBoqArtifactId: string): string {
  const fileName = `bomatic-quick-bom-export-${safePathSegment(projectId)}-${safePathSegment(
    pricedBoqArtifactId
  )}-${randomUUID()}.xlsx`;
...
function toProjectSummary(project: Project): QuickBomExportProjectSummary {
  return {
    id: project.id,
    name: project.name,
...
function toArtifactSummary(artifact: ProjectArtifact): QuickBomExportArtifactSummary {
  return {
    id: artifact.id,
 ...
 */
function toPayloadSummary(payload: MantleExportArtifactPayload): QuickBomExportPayloadSummary {
  return {
...
function toExportSummary(payload: MantleExportArtifactPayload): QuickBomExportSummary {
  return {
    rowCount: payload.rowCount,
    totals: { ...payload.totals },
    warnings: [...payload.warnings],
...
function translateDelegateError(
  message: string
): CreateProjectQuickBomExportPackageResult | null {
 ...
 * `priced_boq` artifact, tenant-scoped on every store/service call. It verifies the
 * Project within its tenant (not_found / wrong_mode, lean summary) BEFORE generating a
 * path or loading the demo category fixture, then generates a unique server-side
 * workbook output path and delegates the load+map+write+persist to
 * {@link createMantleExportArtifact} - passing the committed Honeywell MVP demo Mantle
 * category map as the only categoryByAcceptedSku source. If the delegate throws after a
 * workbook may have been written, the generated path is removed best-effort (never
 * masking the original error) before the known failure is translated into a safe status
 * or anything unexpected is re-thrown. On success it returns lean, serializable
 * summaries of the created artifact (no payload), its payload (no priced lines, amounts,
 * or category map), and the export roll-up. The input, Project, fixture map, and
 * delegate result are never mutated.
 */
export async function createProjectQuickBomExportPackage(
  ...
  const { tenantId, projectId, pricedBoqArtifactId } = input;
