/**
 * Shared configuration-expansion REVIEW core: apply EXPLICIT per-line human
 ...
 * configuration-expansion review step. The single product step is "review a
 * configuration_expansion draft". It verifies the Project (not_found / wrong_mode
 * against the caller-supplied `expectedMode`) and the EXACT draft artifact
 * (existence / configuration_expansion type / draft marker / needs_review status /
 * draft-payload shape), then delegates the per-line review and the new artifact
 * version to the existing deterministic {@link createConfigurationExpansionArtifact}.
 * That delegate re-loads and re-validates the normalized_boq and sku_resolution
 * sources, enforces the sku-approval and approved-rule-pack gates, and applies the
 * pure engineer-review helper (which never auto-accepts: every expansion line needs
 * an explicit decision). The reviewer identity is this core's sole authority
 * (`reviewedBy` from its input); a review timestamp is never settable by a caller
 * and is never generated here. It carries the loaded draft's id/version to the
 * reviewed artifact as source-draft provenance.
 *
 * The ONLY mode-specific behavior is the `project.mode !== expectedMode` gate, so
 * each lane (Quick BoM, RFP BoQ) keeps its own surface while sharing this
 * deterministic core. It does NO catalog/pricing/SKU acceptance/rule-pack work of
 * its own and never imports the artifact-write helper, the draft service, the
 * approval/evidence stores, the raw BoQ loader, pricing, mantle/export, any runner/
 * coordinator/engine/adapter path, or an AI/catalog SDK. It translates the
 * delegate's known failures into discriminated statuses without leaking internal/
 * stack detail, re-throws anything unexpected for the route to map to a safe 500,
 * and returns serializable, lean summaries only (no full lines, acceptedLines,
 * rejectedLines, evidence, originalCells, or full artifact payload). It never
 * mutates its input, the draft artifact, its payload arrays, the decisions, or the
 * delegate's result.
 */
...
// The decision contract is re-exported so lane wrappers and the POST route can stay
// import-pure (the core / Next.js primitives + requireAuth only) while still typing a body.
 ...
 * this core mirrors the literals deliberately to translate them into safe statuses
 * without importing or editing them.
 */
...
/** Input for {@link reviewProjectBoqConfigurationExpansionDraftCore}. */
export interface ReviewProjectBoqConfigurationExpansionDraftInput {
  tenantId: string;
  projectId: string;
...
export interface ProjectBoqConfigExpansionReviewProjectSummary {
  id: string;
  name: string;
  customerName?: string;
  mode: ProjectMode;
...
export interface ProjectBoqConfigExpansionReviewArtifactSummary {
  id: string;
  projectId: string;
...
export interface ProjectBoqConfigExpansionReviewPayloadSummary {
...
export type ProjectBoqConfigExpansionReviewSummary = ConfigurationExpansionArtifactPayload["summary"];

/** Discriminated result of {@link reviewProjectBoqConfigurationExpansionDraftCore}. */
export type ReviewProjectBoqConfigurationExpansionDraftResult =
  | { status: "not_found" }
  | { status: "wrong_mode"; project: ProjectBoqConfigExpansionReviewProjectSummary }
  | { status: "configuration_expansion_draft_not_found" }
  | {
      status: "artifact_not_configuration_expansion";
      artifact: ProjectBoqConfigExpansionReviewArtifactSummary;
    }
  | {
  ...
  | {
      status: "ok";
      artifact: ProjectBoqConfigExpansionReviewArtifactSummary;
      payloadSummary: ProjectBoqConfigExpansionReviewPayloadSummary;
      reviewSummary: ProjectBoqConfigExpansionReviewSummary;
    };

/** The draft-payload fields this core requires before delegating the review. */
...
function toPayloadSummary(
  ...
  const base: ProjectBoqConfigExpansionReviewPayloadSummary = {
...
function translateDelegateError(
  message: string
): ReviewProjectBoqConfigurationExpansionDraftResult | null {
...
/**
 * Review one persisted configuration_expansion DRAFT artifact, tenant-scoped on every
 * store/service call. The Project mode is gated against the caller-supplied
 * `expectedMode` so each lane (Quick BoM, RFP BoQ) keeps its own surface while sharing
 * this deterministic core. `reviewedBy` must be nonblank (a programming invariant;
 ...
 */
export async function reviewProjectBoqConfigurationExpansionDraftCore(
  input: ReviewProjectBoqConfigurationExpansionDraftInput
): Promise<ReviewProjectBoqConfigurationExpansionDraftResult> {
  const {
    tenantId,
    projectId,
