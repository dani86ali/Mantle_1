/**
 * Read-only Quick BoM priced BoQ review projection (Prompt 129): load ONE
 * `priced_boq` artifact and project a lean, serializable line-pricing review view for
 ...
 *
 * This loader is a pure read model. It verifies the Project (not_found / wrong_mode)
 * and the exact priced_boq artifact (priced_boq_not_found / artifact_not_priced_boq /
 * priced_boq_not_reviewable / invalid_priced_boq_payload) within the input tenant,
 * then ALLOWLIST-projects the stored payload into review lines. It never prices, never
 * reads a catalog/GPL/fixture, never persists, never creates an approval, and imports
 * no artifact-write helper, approval store, pricing-creation service, pure pricing
 * math, fixture/catalog/GPL reader, config expansion, export, runner, AI, catalog,
 * engine, coordinator, or adapter. It exposes only the fields an engineer needs to
 * review line pricing before approval: no full payload, no originalCells, no
 * unitListPriceSarBySku (the source price map), no workbook path, no sheet name, no
 * export path, and no replacement/substitution fields. The pricing-authority and
 * configuration-authority traces are allowlisted field-by-field (the pricing-authority
 * workbook path and sheet name are dropped; configuration authority carries no pricing
 * fields or paths). Every array/object in the result is copied, so the projection
 * never aliases the stored artifact payload. Counts are counted in TypeScript from the
 * projected line statuses, never inferred by an LLM.
 */
...
export type QuickBomPricedReviewLineStatus =
  | "priced"
  | "missing_decision"
  | "not_accepted"
  | "missing_price";
...
export interface QuickBomPricedBoqReviewWorkspaceProject {
  id: string;
  tenantId: string;
  name: string;
  customerName?: string;
...
export interface QuickBomPricedBoqReviewWorkspaceArtifact {
  id: string;
  projectId: string;
...
export interface QuickBomPricedBoqReviewTotals {
  currency: string;
  lineCount: number;
...
export interface QuickBomPricedBoqReviewPricingSummary {
  ...
  totals: QuickBomPricedBoqReviewTotals;
}
 ...
 */
export interface QuickBomPricedBoqReviewPricingAuthoritySummary {
  profileId?: string;
  scope?: string;
 ...
 */
export interface QuickBomPricedBoqReviewConfigurationAuthoritySummary {
  scope?: string;
  approvalRecordId?: string;
  rulePackId?: string;
...
export interface QuickBomPricedBoqReviewWorkspacePayload {
  ...
  pricingConfig: ProjectPricingConfig;
  lineCount: number;
  pricingSummary: QuickBomPricedBoqReviewPricingSummary;
  pricingAuthority?: QuickBomPricedBoqReviewPricingAuthoritySummary;
  configurationAuthority?: QuickBomPricedBoqReviewConfigurationAuthoritySummary;
}

/** Deterministic review-state counts, counted from the projected line statuses. */
export interface QuickBomPricedBoqReviewWorkspaceCounts {
...
export interface QuickBomPricedBoqReviewLineAmounts {
  currency: string;
  quantity: number;
...
export interface QuickBomPricedBoqReviewLine {
  sourceFileId: string;
  ...
  description: string;
  quantity: number;
  status: QuickBomPricedReviewLineStatus;
  decisionStatus?: SkuResolutionStatus;
  warning?: string;
  amounts?: QuickBomPricedBoqReviewLineAmounts;
}

/** The lean, serializable priced-BoQ line-review projection returned on `ok`. */
export interface QuickBomPricedBoqReviewWorkspace {
  project: QuickBomPricedBoqReviewWorkspaceProject;
  artifact: QuickBomPricedBoqReviewWorkspaceArtifact;
  payloadSummary: QuickBomPricedBoqReviewWorkspacePayload;
  reviewSummary: QuickBomPricedBoqReviewWorkspaceCounts;
  lines: QuickBomPricedBoqReviewLine[];
}
...
export type LoadQuickBomPricedBoqReviewWorkspaceResult =
  ...
  | { status: "ok"; review: QuickBomPricedBoqReviewWorkspace };
...
function toTotals(raw: unknown): QuickBomPricedBoqReviewTotals | null {
  if (!isPlainObject(raw)) return null;
  const {
    currency,
    lineCount,
...
function toPricingSummary(raw: unknown): QuickBomPricedBoqReviewPricingSummary | null {
  if (!isPlainObject(raw)) return null;
  const {
    inputLineCount,
    pricedLineCount,
...
function toAmounts(raw: unknown): QuickBomPricedBoqReviewLineAmounts | undefined {
 ...
 * along.
 */
function toReviewLine(raw: unknown): QuickBomPricedBoqReviewLine | null {
  if (!isPlainObject(raw)) return null;
  const {
    sourceFileId,
    sourceRowNumber,
  ...
  return {
    ...
    description,
    quantity,
    status: status as QuickBomPricedReviewLineStatus,
...
function toPricingAuthoritySafe(
  raw: unknown
): QuickBomPricedBoqReviewPricingAuthoritySummary | undefined {
  if (!isPlainObject(raw)) return undefined;
  const out: QuickBomPricedBoqReviewPricingAuthoritySummary = {};
...
function toConfigurationAuthoritySafe(
  raw: unknown
): QuickBomPricedBoqReviewConfigurationAuthoritySummary | undefined {
  if (!isPlainObject(raw)) return undefined;
  const out: QuickBomPricedBoqReviewConfigurationAuthoritySummary = {};
  if (typeof raw.scope === "string") out.scope = raw.scope;
  if (typeof raw.approvalRecordId === "string") out.approvalRecordId = raw.approvalRecordId;
...
function parsePricedBoqPayload(
  ...
  const projectedLines: QuickBomPricedBoqReviewLine[] = [];
...
function toProjectSummary(project: Project): QuickBomPricedBoqReviewWorkspaceProject {
  return {
    id: project.id,
...
function toArtifactSummary(artifact: ProjectArtifact): QuickBomPricedBoqReviewWorkspaceArtifact {
  return {
    id: artifact.id,
...
function toReviewCounts(
  lines: readonly QuickBomPricedBoqReviewLine[]
 ...
 * tenant-scoped on every store call. Returns `not_found` when the Project is missing,
 * `wrong_mode` when it is not a Quick BoM project, `priced_boq_not_found` when the
