/**
 * Shared read-only configuration-expansion review WORKSPACE core: load ONE
 * `configuration_expansion` artifact and project a lean, serializable line-review
 * view for the workspace UI. Source of truth:
 ...
 * configuration-expansion review-workspace step. It is a pure read model. It verifies
 * the Project (not_found / wrong_mode against the caller-supplied `expectedMode`) and
 * the EXACT artifact in the same order as the Prompt 91 review service
 * (configuration_expansion_draft_not_found / artifact_not_configuration_expansion /
 * configuration_expansion_draft_not_reviewable /
 * invalid_configuration_expansion_draft_payload) within the input tenant, then
 * ALLOWLIST-projects the stored payload into review lines. It never persists, never
 * auto-accepts/rejects, creates no approval, and imports no artifact-write helper,
 * approval store, pricing, export, runner, AI, catalog, engine, coordinator, adapter,
 * the deterministic config-expansion builder, or the lower-level review helper. The
 * only `@/lib/projects` import is the self-contained, type-only config-expansion-types
 * contract. It exposes only the fields an engineer needs to accept/reject per
 * expansion line: no full payload, no originalCells, no pricing, no file/workbook
 * paths, no evidence sourcePath/sheetName/lineNumber/pageNumber/evidenceNote, and no
 * configuration-authority trace. Evidence is reduced to a count and the citation
 * sourceTypes only. Every array/object in the result is copied, so the projection
 * never aliases the stored artifact payload. Review counts are counted in TypeScript
 * from the projected line origins, never inferred by an LLM.
 *
...
export interface LoadProjectBoqConfigExpansionReviewWorkspaceInput {
  tenantId: string;
  projectId: string;
  /** The exact persisted configuration_expansion artifact to project. */
  artifactId: string;
...
export interface ProjectBoqConfigExpansionReviewWorkspaceProject {
  id: string;
  tenantId: string;
  name: string;
  customerName?: string;
...
export interface ProjectBoqConfigExpansionReviewWorkspaceArtifact {
  id: string;
  projectId: string;
...
export interface ProjectBoqConfigExpansionReviewDraftSummary {
...
export interface ProjectBoqConfigExpansionReviewWorkspacePayload {
  ...
  lineCount: number;
  summary: ProjectBoqConfigExpansionReviewDraftSummary;
}

/** Deterministic review-state counts, counted from the projected line origins. */
export interface ProjectBoqConfigExpansionReviewWorkspaceCounts {
  totalLineCount: number;
  customerLineCount: number;
 ...
 */
export interface ProjectBoqConfigExpansionReviewReviewedCounts {
  customerLineCount: number;
...
export interface ProjectBoqConfigExpansionReviewLine {
  lineId: string;
...
export interface ProjectBoqConfigExpansionReviewWorkspace {
  /** "draft" = editable line review; "reviewed" = read-only recorded decisions. */
  mode: "draft" | "reviewed";
  project: ProjectBoqConfigExpansionReviewWorkspaceProject;
  artifact: ProjectBoqConfigExpansionReviewWorkspaceArtifact;
  /** Present in draft mode only. */
  payloadSummary?: ProjectBoqConfigExpansionReviewWorkspacePayload;
  /** Present in draft mode only. */
  reviewSummary?: ProjectBoqConfigExpansionReviewWorkspaceCounts;
  /** Present in reviewed mode only. */
  reviewedSummary?: ProjectBoqConfigExpansionReviewReviewedCounts;
  lines: ProjectBoqConfigExpansionReviewLine[];
}

/** Discriminated result of {@link loadProjectBoqConfigurationExpansionReviewWorkspaceCore}. */
export type LoadProjectBoqConfigExpansionReviewWorkspaceResult =
  | { status: "not_found" }
  | { status: "wrong_mode" }
  ...
  | { status: "ok"; review: ProjectBoqConfigExpansionReviewWorkspace };
 ...
 */
function toReviewLine(raw: unknown): ProjectBoqConfigExpansionReviewLine | null {
...
function parseDraftPayload(
  ...
  const projectedLines: ProjectBoqConfigExpansionReviewLine[] = [];
...
function toDraftSummarySafe(
  ...
  const out: ProjectBoqConfigExpansionReviewDraftSummary = {};
...
function toReviewedLine(
  raw: unknown,
  decision: "accepted" | "rejected"
): ProjectBoqConfigExpansionReviewLine | null {
...
function parseReviewedPayload(
  ...
  const out: ProjectBoqConfigExpansionReviewLine[] = [];
...
function toReviewedCounts(
  lines: readonly ProjectBoqConfigExpansionReviewLine[]
 ...
 * `configuration_expansion` artifact, tenant-scoped on every store call. The Project
 * mode is gated against the caller-supplied `expectedMode` so each lane (Quick BoM,
 * RFP BoQ) keeps its own surface while sharing this deterministic core. Status order
 * mirrors the Prompt 91 review service: `not_found` when the Project is missing,
 * `wrong_mode` when its mode is not `expectedMode`,
 * `configuration_expansion_draft_not_found` when the artifact is absent,
 * `artifact_not_configuration_expansion` when it is the wrong type. A DRAFT artifact
 * (draft marker present) yields the editable `mode: "draft"` review and requires
 * `needs_review` (else `configuration_expansion_draft_not_reviewable`); a REVIEWED
 * artifact (no draft marker, any status incl. approved) yields the read-only
 ...
 */
export async function loadProjectBoqConfigurationExpansionReviewWorkspaceCore(
  input: LoadProjectBoqConfigExpansionReviewWorkspaceInput
): Promise<LoadProjectBoqConfigExpansionReviewWorkspaceResult> {
