/**
 * OpenAI-adapter RFP HLD intake-question CANDIDATE drafting boundary
 * (Stage 6H-0D).
 *
 * Adapts the provider-neutral HLD intake-question drafting contract
 * (src/lib/projects/project-rfp-hld-intake-questionnaire-drafting-executor.ts) to
 * an OpenAI Responses-style API through an INJECTED minimal client only. This
 * stage wires NO real HTTP client, imports NO provider SDK, uses NO fetch, and
 * reads NO environment: a live client must be supplied by the caller. Each run
 * sends one responses.create call carrying only the deterministic drafting input
 * serialized by the Stage 6H-0D prompt helper plus its fixed system instruction.
 *
 ...
 * forced-function, no response-shape, and no schema fields. Only text output is read
 * (response.output_text, then text blocks in response.output, in order),
...
import type {
  ...
  RfpHldIntakeQuestionnaireDraftingExecutorInput,
...
const MODEL_MESSAGE =
  "A nonblank model is required to build the drafting executor.";
const REQUEST_FAILED_MESSAGE =
  "RFP HLD intake-question drafting request failed.";
const NO_TEXT_MESSAGE =
  "RFP HLD intake-question drafting returned no text output.";
...
export interface OpenAiRfpHldIntakeQuestionnaireDraftingRequest {
  model: string;
  instructions: string;
  input: string;
...
export interface OpenAiRfpHldIntakeQuestionnaireDraftingOutputBlock {
  type?: string;
  text?: string;
}

/** One top-level output item carrying nested content blocks. */
export interface OpenAiRfpHldIntakeQuestionnaireDraftingOutputItem {
  type?: string;
  content?: OpenAiRfpHldIntakeQuestionnaireDraftingOutputBlock[];
}

/** The only response fields this adapter reads. */
export interface OpenAiRfpHldIntakeQuestionnaireDraftingResponse {
  output_text?: string;
  output?: OpenAiRfpHldIntakeQuestionnaireDraftingOutputItem[];
}

/** Minimal Responses-style client contract; tests inject a fake. */
export interface OpenAiRfpHldIntakeQuestionnaireDraftingClient {
  responses: {
    create(
      request: OpenAiRfpHldIntakeQuestionnaireDraftingRequest
    ): Promise<OpenAiRfpHldIntakeQuestionnaireDraftingResponse>;
  };
}

/** Factory configuration; a nonblank model and an injected client are required. */
export interface OpenAiRfpHldIntakeQuestionnaireDraftingConfig {
  model: string;
  /** Required in this stage: no real client is constructed here. */
  client?: OpenAiRfpHldIntakeQuestionnaireDraftingClient;
...
function extractResponseText(
  response: OpenAiRfpHldIntakeQuestionnaireDraftingResponse
): string {
  const parts: string[] = [];
...
/**
 * Build an RfpHldIntakeQuestionnaireDraftingExecutor backed by an injected OpenAI
 ...
 */
export function createOpenAiRfpHldIntakeQuestionnaireDraftingExecutor(
  config: OpenAiRfpHldIntakeQuestionnaireDraftingConfig
): RfpHldIntakeQuestionnaireDraftingExecutor {
  const model =
  ...
  return async (input: RfpHldIntakeQuestionnaireDraftingExecutorInput) => {
    const prompt = buildRfpHldIntakeQuestionnaireDraftingRequest(
      input.draftingInput
    );
    const request: OpenAiRfpHldIntakeQuestionnaireDraftingRequest = {
      model,
    ...
    let response: OpenAiRfpHldIntakeQuestionnaireDraftingResponse;
    try {
